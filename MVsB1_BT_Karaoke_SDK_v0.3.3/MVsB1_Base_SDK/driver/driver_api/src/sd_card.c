...
#define SDIO_CLK_SYSTEM_DIV				(Clock_PllFreqGet() / 50000000 + ((Clock_PllFreqGet() % 50000000) ? (1) : (0)))
...
#endif
	SDIO_SysToSdioDivSet(SDIO_CLK_SYSTEM_DIV - 1);
	SDIO_Init();
	SDIO_ByteModeEnable();
...
SD_CARD_ERR_CODE SDCard_Identify(void)
{
	...
	DBG("resp:%X\n", resp[11]);
	if(i == 0 || (resp[11] & 0x07) > 3)
	{
		DBG("Csd error!\n");
	}
	uint32_t TargetClk = 0;//@ K
	uint32_t SdioBaseClk	= Clock_PllFreqGet()  / (1000 * SDIO_CLK_SYSTEM_DIV);//@ K
	if((resp[11] & 0x07) == 0)//base 0.1M
	{
		TargetClk = 100;
	}
	else if((resp[11] & 0x07) == 1)	//base 1M
	{
		TargetClk = 1000;
	}
	...
	else if((resp[11] & 0x07) == 3)	//base 100M
	{
		TargetClk = 1000000;
	}

	if(i > 0x0C)//6.0M,7.0M,8.0M
	{
		TargetClk *= 5 + i - 0x0C;
	}
	else if(i > 0x03)//1.5M,2.0M,2.5M,3.0M,3.5M,4.0M,4.5M,5.0M,5.5M
	{
		TargetClk +=  (TargetClk * (i - 0x03) * 5) / 10;
	}
	else if(i > 0x01)//1.2M, 1.3M
	{
		TargetClk +=  (TargetClk * i) / 10;
	}

	for(i = 0; i<= 0x0F; i++)
	{
		SdioBaseClk /= 2;
		if(SdioBaseClk <= TargetClk)
			break;
	}
	SDCard.MaxTransSpeed = i;

	DBG("Max:%luK Set:%u Clk:%luK \n", TargetClk, SDCard.MaxTransSpeed, SdioBaseClk);
	//进入transfer state
