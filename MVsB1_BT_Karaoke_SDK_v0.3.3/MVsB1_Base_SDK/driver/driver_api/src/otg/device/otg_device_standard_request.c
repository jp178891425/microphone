...
int8_t UsbDeviceBulkReceiveIntTransactionRequest(void)
{
    uint16_t temp;

    #define otg_CommonUSB_Index *(uint32_t *)0x40000000

   // WriteOTGReg8(otg_CommonUSB_Index, TYPE_BULK_IN);

    temp = 100;//ReadOTGReg16(otg_IndexedCSR_TxCSR);

    if(temp & (1 << 2))
    {

        // ClrBitOTGReg8(otg_IndexedCSR_TxCSR, (1 << 2));
...
void OTG_DeviceModeSel(uint8_t Mode,uint16_t UsbVid,uint16_t UsbPid)
{
	...
	gDeviceString_SerialNumber      = "20210326";						//max length: 32bytes
#ifdef CFG_FUNC_USB_MIDE_EN
	if(Mode==AUDIO_USB_MIDI)
	{
		midiDevice = 1;
	}
	else
	...
	switch(Setup[1])
	{
		...
		case USB_REQ_SET_CONFIGURATION:
			...
			//	DBG("Audio_ISO sam test\n");
				...
				{
					uint8_t usb_temp[8];
					uint32_t pTransferLen;
					memset(usb_temp,0,sizeof(usb_temp));
					OTG_EndpointInterruptEnable(DEVICE_BULK_OUT_EP, MidiBulkRecvCallBack);
					OTG_DeviceBulkReceive(DEVICE_BULK_OUT_EP, (uint8_t*)usb_temp, 8,  &pTransferLen,1);
