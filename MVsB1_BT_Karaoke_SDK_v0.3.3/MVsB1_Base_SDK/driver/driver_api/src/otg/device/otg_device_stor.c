...
void DeviceStorReadFmtCapacity(void)
{
	...
	{
		uint32_t *pp = (uint32_t*)&gFmtCapacityData[4];
		*pp = CpuToBe32(Temp);
	}
...
void DeviceStorReadCapacity(void)
{
	...
	{
		uint32_t *pp = (uint32_t*)&gCapacityData[0];
		*pp = CpuToBe32(Temp);
	}
...
{
	TIMER Timer;
	uint32_t *p32 = (uint32_t*)(&gCBW[17]);
	uint16_t *p16 = (uint16_t*)(&gCBW[22]);
	uint32_t ReqLBA;// = Be32ToCpu(*(uint32_t*)(&gCBW[17]));
	uint16_t ReqCnt;// = Be16ToCpu(*(uint16_t*)(&gCBW[22]));

	ReqLBA = Be32ToCpu(*p32);
	ReqCnt = Be16ToCpu(*p16);
...
void DeviceStorModeSense(void)
{
	...
	{
		uint32_t* pp = (uint32_t*)&gModeSenseAllPage[4];
		*pp = CpuToBe32(SDCard_CapacityGet());
	}
...
void OTG_DeviceStorProcess(void)
{
	...
	switch(gCBW[15])
	{
		...
		default:
			//DBG("default\n");
			tempp = ((gCBW[8]) | (gCBW[9]<<8) | (gCBW[10]<<16) | (gCBW[11]<<24));
			if(tempp > 0)
			{
