...
void AudioDAC_Init(DAC_MODULE DACModule, uint32_t SampleRate, void* Buf1, uint16_t Len1, void* Buf2, uint16_t Len2)
{
	...
	if(DAC0Enable)
	{
		...
		AudioDAC_DsmOutdisModeSet(DAC0, 0);//关闭automute功能，有些场景下会有pop声
	...
	if(DAC1Enable)
	{
		...
		AudioDAC_DsmOutdisModeSet(DAC1, 0);//关闭automute功能，有些场景下会有pop声
...
void AudioDAC_InitLowEnergy(DAC_MODULE DACModule, uint32_t SampleRate, void* Buf1, uint16_t Len1, void* Buf2, uint16_t Len2)
{
	...
	{
		AudioDAC_ChannelEnable(DAC0);
		if(AudioDAC_PowerOnLowEnergy(DAC0))
		{
			DBG("DAC0 LowEnergy poweron OK!\n");
		}
	}
	else if(DACModule == DAC1)
	{
		AudioDAC_ChannelEnable(DAC1);
		if(AudioDAC_PowerOnLowEnergy(DAC1))
		{
			DBG("DAC1 LowEnergy poweron OK!\n");
		}
	}
	else
	{
		AudioDAC_AllChannelEnable();
		if(AudioDAC_AllPowerOnLowEnergy())
		{
			DBG("DAC LowEnergy All poweron OK!\n");
		}
	}
	...
	if(DAC0Enable)
	{
		AudioDAC_IBSelect(DAC0, 1, 1);
	}
	if(DAC1Enable)
	{
		AudioDAC_IBSelect(DAC1, 1, 1);
	}
}
