...
#ifdef BT_VOLUME_SYNC_TO_PHONE
uint8_t gAvrcpVolSyncFlag = 0;
#endif
...
#endif
				if((GetA2dpState() == BT_A2DP_STATE_CONNECTED)&&(memcmp(GetBtManager()->remoteAddr, param->params.bd_addr, 6) != 0))
				{
					//a2dp已连接,和avrcp连上的设备不一致,则断开
					BTHciDisconnectCmd(param->params.bd_addr);
...
#include "bt_stack_api.h"
		...
		case BT_STACK_EVENT_AVRCP_ADV_PLAY_STATUS_CHANGED:
			{
				...
				#ifndef BT_FAST_POWER_ON_OFF_FUNC
				extern void SetA2dpState(BT_A2DP_STATE state);
		...
		case BT_STACK_EVENT_AVRCP_ADV_TRACK_INFO:
			...
			//20240515
			{
				extern uint32_t gGetBtMediaInforCnt;
				gGetBtMediaInforCnt = 1;
			}
#endif
			break;
...
#if (BT_AVRCP_VOLUME_SYNC == ENABLE)
#ifndef BT_VOLUME_SYNC_TO_PHONE
