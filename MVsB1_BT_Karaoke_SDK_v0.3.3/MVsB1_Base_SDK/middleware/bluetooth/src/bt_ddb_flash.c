...
void BtDdb_LastBtAddrErase(void)
{
	...
	APP_DBG("BtDdb_LastBtAddrErase\n");
	count = DdbFindRecord((const uint8_t*)btManager.btDdbLastAddr);
	if (count == DDB_NOT_FOUND)
	{
		return;
	}
...
uint32_t BtDdb_UpgradeTwsInfor(uint8_t *BtTwsAddr)
{
	...
	if (count == DDB_NOT_FOUND)
	{
        ...
        memcpy(btManager.btLinkDeviceInfo[0].device.bdAddr,BtTwsAddr, 6);
        btManager.btLinkDeviceInfo[0].device.keyType = 1;
        btManager.btLinkDeviceInfo[0].device.pinLen = 4;
