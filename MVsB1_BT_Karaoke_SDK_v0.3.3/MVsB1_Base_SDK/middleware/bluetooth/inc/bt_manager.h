...
typedef struct _BT_ACCESS_MODE_ST
{
	uint8_t 				accessMode;
	uint16_t				delayTime;
}BT_ACCESS_MODE_ST;
...
//	uint8_t					btDevName[BT_NAME_SIZE];
	...
	BT_ACCESS_MODE_ST		btAccessModeSt;
	uint8_t					btScanDisable;		//手动开关蓝牙可见性 1=关闭可见性; 0=默认配置
...
//tws
	...
	bool					TwsFunctionEnable;
	uint8_t*				TwsMemP;
	int16_t*                TwsAudioCoreBuf;
	bool					TwsAudioCoreExitFlag;
#endif
...
//	uint8_t					remoteAvrcpAddr[BT_ADDR_SIZE];
	...
	uint32_t				bleAdvDataSetDelayCnt;
...
/**
 * @brief  Set Bt Scan disable
 * @param  flag: 1=bt scan disable;
 * @return NONE
 * @Note
 *
 */
