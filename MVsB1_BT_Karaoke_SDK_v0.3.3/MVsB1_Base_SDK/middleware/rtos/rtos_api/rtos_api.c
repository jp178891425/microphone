...
void *osPortMalloc_1(uint16_t osWantedSize)
{
	...
	vPortExitCritical();
#ifdef OS_MEM_DEBUG
	DBG("Malloc:%08x %d %d \n",ospvReturn, (int)osWantedSize, (int)xPortGetFreeHeapSize());
#endif
	return ospvReturn;
}
...
#ifdef CFG_FUNC_RTOS_IDLE_TO_AUDIOCORE_MSG
		{
			#include "app_message.h"
			extern bool AudioCoreDataCheck(void);
			extern MessageHandle GetAudioCoreServiceMsgHandle(void);
			if(AudioCoreDataCheck())
			{
