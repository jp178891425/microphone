...
extern void tws_sound_remind_cmd(uint8_t *p);
#endif
...
typedef struct _RemindSoundServiceContext
{
	...
	uint8_t				RemindSysBuf[REMIND_SOUND_ID_LEN * CFG_PARAM_REMIND_LIST_SYS];//系统提示音队列
	MCU_CIRCULAR_CONTEXT	RemindSysCircular;

	uint8_t				RemindOrderBuf[REMIND_SOUND_ID_LEN * CFG_PARAM_REMIND_LIST_MAX];//App内提示音队列
	MCU_CIRCULAR_CONTEXT	RemindOrderCircular;

	uint32_t			Attr;

	uint8_t				RequestRemind[REMIND_SOUND_ID_LEN];//可被打断/抢占的提示音条目
...
void RemindSoundReinit(void)
{
	...
	MCUCircular_Config(&RemindSoundServiceCt.RemindSysCircular, RemindSoundServiceCt.RemindSysBuf, REMIND_SOUND_ID_LEN * CFG_PARAM_REMIND_LIST_SYS);
	RemindSoundServiceCt.Attr = REMIND_ATTR_NONE;
	MCUCircular_Config(&RemindSoundServiceCt.RemindOrderCircular, RemindSoundServiceCt.RemindOrderBuf, REMIND_SOUND_ID_LEN * CFG_PARAM_REMIND_LIST_MAX);
}
...
#ifdef BT_TWS_SUPPORT
			//关机提示音在本地播放
			if(memcmp(RemindItem,SOUND_REMIND_GUANJI,REMIND_SOUND_ID_LEN) == 0)
				TwsCfg.IsRemindSyncEn = 0;
			else
...
static bool RemindSoundServicePlayItem(void)
{
	...
	while((MCUCircular_GetDataLen(&RemindSoundServiceCt.RemindSysCircular)))//有系统提示音申请序列
	{
		//做检查。
		if(RemindSoundServiceReadItemInfo((uint8_t*)&(RemindSoundServiceCt.RemindSysCircular.CircularBuf[RemindSoundServiceCt.RemindSysCircular.R])))
		{
				MCUCircular_AbortData(&RemindSoundServiceCt.RemindSysCircular, REMIND_SOUND_ID_LEN);
				RemindSoundServiceCt.Attr = REMIND_ATTR_SYS;
				ItemCheck = TRUE;
				break;
		}
		else
	...
	while((MCUCircular_GetDataLen(&RemindSoundServiceCt.RemindOrderCircular)))//有提示音申请序列
	{
		if(RemindSoundServiceReadItemInfo((uint8_t*)&(RemindSoundServiceCt.RemindOrderCircular.CircularBuf[RemindSoundServiceCt.RemindOrderCircular.R])))
		{
				APP_DBG("Order remind:Play Item\n");//播放新提示音有效。
				MCUCircular_AbortData(&RemindSoundServiceCt.RemindOrderCircular, REMIND_SOUND_ID_LEN);
				RemindSoundServiceCt.Attr = REMIND_ATTR_ORDER;
				ItemCheck = TRUE;
				break;
		}
		else
	...
	if(!ItemCheck)
	{
		if(RemindSoundServiceCt.RequestRemind[0] && RemindSoundServiceReadItemInfo(RemindSoundServiceCt.RequestRemind))
		{
			...
			RemindSoundServiceCt.Attr = REMIND_ATTR_NORMAL;
			ItemCheck = TRUE;
		}
		else
		{
	...
	while(1)
	{
		...
		switch(msgRecv.msgId)
		{
			...
			case MSG_TASK_STOP:
				...
				break;
#ifdef BT_TWS_SUPPORT
			case MSG_REMIND_SOUND_WAIT_TWS_INIT:
				if(g_tws_need_init == 2 && GetBtManager()->twsState == BT_TWS_STATE_CONNECTED && wait_cnt < 2000)
				{
					vTaskDelay(1);
					msgSend.msgId = MSG_REMIND_SOUND_WAIT_TWS_INIT;
			...
			case MSG_REMIND_SOUND_PLAY_REQUEST://响应 有效的提示音播放请求，
				if(!MCUCircular_GetDataLen(&RemindSoundServiceCt.RemindSysCircular) && !MCUCircular_GetDataLen(&RemindSoundServiceCt.RemindOrderCircular) && !RemindSoundServiceCt.RequestRemind[0])
				{//没有 阻塞或非阻塞提示音条目
					break;
				}
#ifdef BT_TWS_SUPPORT
				...
				else if(RemindSoundServiceCt.RemindState == REMIND_PLAY && !(RemindSoundServiceCt.Attr & (REMIND_ATTR_ORDER | REMIND_ATTR_SYS)))
				{//非阻塞 可打断
			...
			case MSG_REMIND_SOUND_PLAY: //提示音播放;此消息要求发送者确保解码器ready。
				if((SoftFlagGet(SoftFlagDecoderRemind) && (RemindSoundServiceCt.RemindState == REMIND_WAIT_DECODER)) || MCUCircular_GetDataLen(&RemindSoundServiceCt.RemindSysCircular))
				{
					APP_DBG("Remind:Play\n");
...
#ifdef BT_TWS_SUPPORT
					if((GetBtManager()->twsState == BT_TWS_STATE_CONNECTED)&&(TwsCfg.IsRemindSyncEn))
					{
						WaitingMsg = 400;
					}
					else
...
bool RemindSoundServiceItemRequest(char *SoundItem, uint32_t Attr)
{
	...
	if(SoftFlagGet(SoftFlagNoRemind)
			|| !(SoftFlagGet(SoftFlagDecoderRun) || (Attr & REMIND_ATTR_SYS)))
	{
		msgSend.msgId		= MSG_REMIND_SOUND_PLAY_REQUEST_FAIL;
		MessageSend(RemindSoundServiceCt.parentMsgHandle, &msgSend);
		ret = FALSE;
			...
			|| ((Attr & REMIND_ATTR_SYS) && MCUCircular_GetSpaceLen(&RemindSoundServiceCt.RemindSysCircular) <= REMIND_SOUND_ID_LEN)
			|| ((Attr & REMIND_ATTR_ORDER) && MCUCircular_GetSpaceLen(&RemindSoundServiceCt.RemindOrderCircular) <= REMIND_SOUND_ID_LEN))
	{
		return FALSE;
	}
	else
	{
		if(Attr & REMIND_ATTR_SYS)
		{
			DBG("Test:Remind Sys Request\n");
			MCUCircular_PutData(&RemindSoundServiceCt.RemindSysCircular, SoundItem, REMIND_SOUND_ID_LEN);
		}
		else if(Attr & REMIND_ATTR_ORDER)
		{
			MCUCircular_PutData(&RemindSoundServiceCt.RemindOrderCircular, SoundItem, REMIND_SOUND_ID_LEN);
		}
		else
		{
		...
		if((IsAudioPlayerMute() == FALSE)&& (!(SoftFlagGet(SoftFlagDecoderRemind))))
		{
			AudioPlayerMute();
			#ifdef BT_TWS_SUPPORT
		...
		//当前正在进行队列播放时，实际依赖于 提示音播放结束MSG_DECODER_STOPPED后播空后续连播。
		if(SoftFlagGet(SoftFlagDecoderRun))
		{
...
//TRUE 提示音正播放 FALSE：未播放
bool RemindSoundServiceIsPlaying(void)
{
	if((RemindSoundServiceCt.Attr & (REMIND_ATTR_NORMAL | REMIND_ATTR_ORDER | REMIND_ATTR_SYS)))
	{
		return TRUE;
	}
	else
...
{
	return !((RemindSoundServiceCt.Attr & REMIND_ATTR_SYS) || MCUCircular_GetDataLen(&RemindSoundServiceCt.RemindSysCircular));
}
...
extern TIMER EffectChangeTimer;
extern MessageHandle GetBtHfMessageHandle(void);
uint32_t RemindSoundServiceFillStreamCallback(void *buffer, uint32_t length)
{
	int32_t RemainBytes;
	int32_t ReadBytes;
	...
	if(ReadBytes == 0)
	{
		if(RemindSoundServiceCt.RemindState != REMIND_STOPPING )//避免消息时差，反复发起停止。
		{
			...
			if(GetHfpState() == BT_HFP_STATE_INCOMING && (GetSystemMode() == AppModeBtHfPlay))
			{
				...
				if(SoftFlagGet(SoftFlagWaitBtRemindEnd))
				{
					MessageContext		msgSend;
					//通话时,连上蓝牙,播放连接提示音结束后,再进入通话模式
					SoftFlagDeregister(SoftFlagWaitBtRemindEnd);
					if(SoftFlagGet(SoftFlagDelayEnterBtHf))
					{
						...
						msgSend.msgId = MSG_DEVICE_SERVICE_ENTER_BTHF_MODE;
						MessageSend(GetMainMessageHandle(), &msgSend);
						APP_DBG("RemindSoundServiceFillStreamCallback return, and enter bthf mode\n");
						return 0;
					}
				}
				...
				MessageContext msgSend;
				MessageHandle msgHandle;
				msgHandle = (MessageHandle)GetBtHfMessageHandle();
