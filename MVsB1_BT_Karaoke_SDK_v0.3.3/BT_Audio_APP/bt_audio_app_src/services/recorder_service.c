...
typedef struct _MediaRecorderContext
{
	...
	uint32_t			sRecLost;
	uint32_t			sRecBlockMax;//实际设备最大block值，MEDIA_RECODER_IO_BLOCK_TIME 的兼容性设置参考。
#endif
	uint32_t			sRecordingTime ; // unit ms
	uint32_t			sRecordingTimePre ;
#ifdef CFG_PARA_REC_SAFETY_TIME
	...
	FFILE				*RecordFile;
#elif defined(CFG_FUNC_RECORD_FLASH)
	uint16_t			FileList[CFG_PARA_REC_MAX_FILE_NUM]; //录音文件队列
	uint16_t			FileNum;//录音文件计数器。
	uint16_t			FileIndex;//准备录音文件编号
	uint16_t			FlashBlock;//准备录音分区index
	uint32_t			FileWriteLen;
#endif
...
static bool FileIndexInit(void)
{
	...
	uint16_t i, RecFileList = 0;
	REC_BLOCK_HEADER Block_Header;

	RecorderCt->FileIndex = 0;
	RecorderCt->FileNum = 0;
	RecorderCt->FlashBlock = CFG_PARA_REC_MAX_FILE_NUM;//找出最小值
	for(i = 0; i < CFG_PARA_REC_MAX_FILE_NUM; i++)
	{
		SPI_Flash_Read(EXTERN_BLOCK_SIZE * i + REC_BLOCK_ADDR, &Block_Header, sizeof(REC_BLOCK_HEADER));
		{
			if(Block_Header.Index > 0 && Block_Header.Index <= FILE_NAME_MAX)
			{
				RecorderCt->FileNum++;
				DBG("File:%d Size:0x%x Block:%d\n", Block_Header.Index, Block_Header.FileSize, i);
				if(Block_Header.Index > RecFileList)
				{
					RecFileList = Block_Header.Index;//找已录音文件更大编号
				}
			}
			else if(Block_Header.Index == FLASH_NONE && i < RecorderCt->FlashBlock)
			{
				RecorderCt->FlashBlock = i;
			}
		}
	}
	if(RecFileList < FILE_NAME_MAX && RecorderCt->FileNum < CFG_PARA_REC_MAX_FILE_NUM && RecorderCt->FlashBlock < CFG_PARA_REC_MAX_FILE_NUM)
	{
		RecorderCt->FileIndex = RecFileList + 1;//最大编号下一个编号作为新录音文件编号
		DBG("FileIndex:%d\n", RecorderCt->FileIndex);
		return TRUE;
	}
	DBG("FlashBlock:%d\n", RecorderCt->FlashBlock);
	return FALSE;
}

void FlashBlockDel(uint8_t BlockIndex)
{
	uint32_t Addr;
	if(BlockIndex >= CFG_PARA_REC_MAX_FILE_NUM)
		return;
#if CFG_PARA_REC_MAX_FILE_NUM == 1
	SPI_Flash_Erase_Chip();
#else
	Addr = BlockIndex * EXTERN_BLOCK_SIZE;
	while(Addr < (BlockIndex + 1) * EXTERN_BLOCK_SIZE)
	{
		if(Addr % (64* 1024))
		{
			SPI_Flash_Erase(SECTOR_ERASE, Addr / (4096));
			Addr += 4096;
		}
		else if(Addr + 64* 1024 <= (BlockIndex + 1) * EXTERN_BLOCK_SIZE)
		{
			SPI_Flash_Erase(BLOCK_ERASE, Addr / (64* 1024));
			Addr += 64 * 1024;
		}
	}
#endif
...
void FlashBlockWrite(uint8_t BlockIndex, uint32_t BlockOffset, uint8_t *buf, uint32_t Len)
{
	SPI_Flash_Write(BlockOffset + EXTERN_BLOCK_SIZE * BlockIndex, buf, Len);
}

static void CloseFlashBlock(uint16_t Index, uint32_t Len)
{
	REC_BLOCK_HEADER Block_Header;
	SPI_Flash_Read(EXTERN_BLOCK_SIZE * Index + REC_BLOCK_ADDR, (uint8_t *)&Block_Header, sizeof(REC_BLOCK_HEADER));
	if(Block_Header.FileSize == FLASH_NONE)
	{
		Block_Header.FileSize = Len;
		SPI_Flash_Write(EXTERN_BLOCK_SIZE * Index + REC_BLOCK_SIZE_ADDR, (uint8_t *)(&Block_Header.FileSize), REC_BLOCK_SIZE_LEN);
		DBG("Addr:0x%x Size:0x%x\n", EXTERN_BLOCK_SIZE * Index + REC_BLOCK_SIZE_ADDR, Len);
	}
//	else
//		DBG("File size Err0x%x\n", );
	return;
}
#endif //defined(CFG_FUNC_RECORD_FLASH) //CFG_FUNC_RECORD_SD_UDISK
...
static bool MediaRecorderOpenDataFile(void)//uint8_t SongType)
{
...
#elif defined(CFG_FUNC_RECORD_FLASH)
	if(RecorderCt->FlashBlock >= CFG_PARA_REC_MAX_FILE_NUM || RecorderCt->FileIndex == 0)
	{
		APP_DBG("Open file error!\n");
		return FALSE;
	}
	...
	{
		REC_BLOCK_HEADER Block_Header;
		Block_Header.Index = RecorderCt->FileIndex;
		FlashBlockWrite(RecorderCt->FlashBlock, REC_BLOCK_INDEX_ADDR, (uint8_t*)&(Block_Header.Index), REC_BLOCK_INDEX_LEN);
		APP_DBG("Open File ok!0x%x\n", REC_BLOCK_INDEX_ADDR);
		RecorderCt->FileWriteLen = 0;
		return TRUE;
	}
#endif
}
...
uint16_t MediaRecorderDataSet(void* Buf, uint16_t Len)
{
	...
	if(RecorderCt->RecorderOn)
	{
		...
		{
			APP_DBG("*");//音频丢失警告，后续编码阻塞。
#ifdef CFG_FUNC_DEBUG_EN
			RecorderCt->sRecLost += Len;
#endif
			return 0;
		}
	...
	if(RecorderCt != NULL && RecorderCt->RecorderOn)
	{
		...
		if(Len / (MEDIA_RECORDER_CHANNEL * 2) + RecorderCt->RemainSample  >= RecorderCt->SamplePerFrame)
		{
			...
			if(!encoder_encode(RecorderCt->EncodeBuf, RecorderCt->Mp3OutBuf, &Len))// len=313
			{
				...
				{
#ifdef CFG_FUNC_DEBUG_EN
					RecorderCt->sRecLost += RecorderCt->SamplePerFrame;
#endif
...
void MediaRecorderStopProcess(void)
{
...
#elif defined(CFG_FUNC_RECORD_FLASH)
		if(RecorderCt->FlashBlock < CFG_PARA_REC_MAX_FILE_NUM)
		{
			CloseFlashBlock(RecorderCt->FlashBlock, RecorderCt->FileWriteLen);
			RecorderCt->FlashBlock = CFG_PARA_REC_MAX_FILE_NUM;
		}
#endif
	}
	...
	if(Times)
	{
		APP_DBG("Recorded %u M %u S Lost:%lu S BlockMax:%lu mS\n", (int)(Times / 60), (int)(Times % 60),
				(RecorderCt->sRecLost + RecorderCt->SampleRate - 1 ) / RecorderCt->SampleRate,
				RecorderCt->sRecBlockMax);
	}
	RecorderCt->sRecLost = 0;
	RecorderCt->sRecBlockMax = 0;
#endif
	...
	if(!ret)
	{
		...
		if(ResourceValue(AppResourceCard))
		{
			...
			if(SDCard_Init() == 0)
			{
				...
				{
					f_mbr_start(i);
					if((res = f_mount(&RecorderCt->MediaFatfs, MEDIA_VOLUME_STR_C, 1)) == FR_OK)					
					{					
						break;
					}
				}
...
static bool MediaRecorderDataProcess(void)
{
	...
	switch(RecorderCt->RecState)
	{
		case RecorderStateWriteFile:
			...
			osMutexLock(RecorderCt->FifoMutex);
#ifdef CFG_FUNC_DEBUG_EN
			DataTime = MCUCircular_GetDataLen(&RecorderCt->FileWCircularBuf) / (MEDIA_RECORDER_BITRATE / 8);
			TimeOutSet(&WriteTimer, 1);
#endif
...
#elif defined(CFG_FUNC_RECORD_FLASH)
			if(RecorderCt->FileWriteLen < FLASH_FILE_MAX)
			{
				if(RecorderCt->FileWriteLen + Len > FLASH_FILE_MAX)
				{
					Len = FLASH_FILE_MAX - RecorderCt->FileWriteLen;
				}
				FlashBlockWrite(RecorderCt->FlashBlock, RecorderCt->FileWriteLen, RecorderCt->WriteBuf, Len);
				RecorderCt->FileWriteLen += Len;
			}
			else
#endif
			{
...
#endif
				{
					APP_DBG("No space for this record\n");
				}
				MessageContext		msgSend;
...
#ifdef CFG_FUNC_DEBUG_EN
			DataTime += PastTimeGet(&WriteTimer);
			if(DataTime > RecorderCt->sRecBlockMax)
			{
				RecorderCt->sRecBlockMax = DataTime;
			}
#endif
		...
		case RecorderStateSaveFlie:
			...
			if(((uint64_t)RecorderCt->sRecSamples * 1000) / RecorderCt->SampleRate - RecorderCt->SyncFileMs >= CFG_PARA_REC_SAFETY_TIME
				|| RecorderCt->SyncFileMs == 0 )
			{
#ifdef CFG_FUNC_DEBUG_EN
				DataTime = MCUCircular_GetDataLen(&RecorderCt->FileWCircularBuf) / (MEDIA_RECORDER_BITRATE / 8);
...
#endif//CFG_PARA_REC_SAFETY_TIME
...
//		}
	...
	//编码输出buf
	RecorderCt->Mp3OutBuf = (uint8_t*)osPortMalloc(ENCODER_MP3_OUT_BUF_SIZE);
...
void ShowRecordingTime(void)
{
	...
	{
	  APP_DBG("%u.mp3 Recording(%lds)\n",RecorderCt->FileIndex, RecorderCt->sRecordingTime / 1000);
	  RecorderCt->sRecordingTimePre = RecorderCt->sRecordingTime;
	}
#elif defined(CFG_FUNC_RECORD_FLASH)
	if((RecorderCt->sRecordingTime / 1000 - RecorderCt->sRecordingTimePre / 1000 >= 1))
	{
	  APP_DBG("%u.mp3 Recording(%lds Max%lds)\n",RecorderCt->FileIndex, RecorderCt->sRecordingTime / 1000, ((EXTERN_BLOCK_SIZE - sizeof(REC_BLOCK_HEADER)) * 8) / (1000 * MEDIA_RECORDER_BITRATE));
	  RecorderCt->sRecordingTimePre = RecorderCt->sRecordingTime;
	}
#endif 
}
