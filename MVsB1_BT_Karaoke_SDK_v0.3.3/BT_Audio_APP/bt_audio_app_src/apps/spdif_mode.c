...
//	memset(SpdifPlayCt->Source1Buf_Spdif, 0, SPDIF_SOURCE_LEN);
	...
	SpdifPlayCt->AudioCoreSpdif->AudioSource[SPDIF_SOURCE_NUM].IsSreamData = FALSE;
#else
...
static void SpdifPlayModeStarting(uint16_t msgId)
{
...
#endif
	{
		...
		{
#ifdef BT_TWS_SUPPORT
			vTaskDelay(110);//TWS 模式提示音同步  实测适配
#endif
			if(GetSystemMode() == AppModeOpticalAudioPlay)
			{
				#ifdef BT_TWS_SUPPORT
				...
				#endif
				{
					if(!RemindSoundServiceItemRequest(SOUND_REMIND_GXIANMOD, REMIND_ATTR_NORMAL))//插播提示音
			...
			else
			{
				...
				#endif
				{
					if(!RemindSoundServiceItemRequest(SOUND_REMIND_TZHOUMOD, REMIND_ATTR_NORMAL))
...
void SpdifDataCarry(void)
{
	...
	while(cnt--)
	{
		...
		SPDIF_SPDIFDatatoAudioData((int32_t *)pcmBuf, (MAX_FRAME_SAMPLES - SpdifPlayCt->pcmRemainLen) * 8, (int32_t *)pcmBuf, SPDIF_WORDLTH_16BIT, &SpdifPlayCt->AudioInfo);
		pcm_len = SpdifPlayCt->AudioInfo.output_length;
#else
		DMA_CircularDataGet(PERIPHERAL_ID_SPDIF_RX, pcmBuf, MAX_FRAME_SAMPLES * 8);
		//由于从32bit转换为16bit，buf可以使用同一个，否则要独立申请。
		SPDIF_SPDIFDatatoAudioData((int32_t *)pcmBuf, MAX_FRAME_SAMPLES * 8, (int32_t *)pcmBuf, SPDIF_WORDLTH_16BIT, &SpdifPlayCt->AudioInfo);
		pcm_len = SpdifPlayCt->AudioInfo.output_length;
#endif
