...
bool IsAudioPlayerMute(void);
	...
	bool FlashPlayerInit(void);
	uint32_t FlashReadFile(void *buffer, uint32_t length);
#endif
...
bool FolderExclude_callback(FILINFO *finfo)
{
	...
	if(MvaCountEn)
	{
		if(GetSystemMode() == AppModeCardAudioPlay && MvaCountEn > 1)
		{
			SoftFlagRegister(SoftFlagMvaInCard);
		}
		else if(GetSystemMode() == AppModeUDiskAudioPlay && MvaCountEn > 1)
		{
...
static bool HardwareInit(DEV_ID DevId)
{
	...
	switch(DevId)
	{
		...
		case DEV_ID_SD:
		{
			...
			{
				Retry--;
				for(i=0; i<FS_MAX_SEARCH; i++)
				{
					f_mbr_start(i);
					if (f_mount(&gpMediaPlayer->gFatfs_sd, current_vol, 1) == FR_OK)				
					{	
						res = 1;
						break;
					}
...
//	memset(gpMediaPlayer, 0, sizeof(MEDIA_PLAYER));
	...
	if(GetSystemMode() == AppModeUDiskAudioPlay || GetSystemMode() == AppModeCardAudioPlay)
	{
...
#ifdef CFG_FUNC_NON_STANDARD_EN
		ffpresearch_fattrib_set(0x00);
#endif
...
#ifdef	FUNC_SPECIFY_FOLDER_PLAY_EN
		if(gpMediaPlayer->CurPlayMode == PLAY_MODE_REPEAT_FOLDER)
			gpMediaPlayer->CurPlayTime = GetCurFoldersSongPlayTime();
#endif
		...
		memset(&gpMediaPlayer->PlayerFolder, 0, sizeof(ff_dir));
		// 打开播放文件夹，默认全盘播放
		// 文件夹相关操作后续补充
	}
...
#elif defined(CFG_FUNC_RECORD_FLASH)
		if(!FlashPlayerInit())//flash无有效文件
		{
			APP_DBG("open error");
			return FALSE;
		}
		gpMediaPlayer->FlashMemHandle.addr = NULL;//不做缓冲，由解码器直接获取
		gpMediaPlayer->FlashMemHandle.mem_capacity = 0;
		gpMediaPlayer->FlashMemHandle.mem_len = 0;
		gpMediaPlayer->FlashMemHandle.p = 0;
#endif//CFG_FUNC_RECORD_FLASH
		gpMediaPlayer->CurFolderIndex = 1; // 不在加速器计算内
		gpMediaPlayer->ValidFolderSumInDisk = 1;
		gpMediaPlayer->CurFileIndex = 1;//倒序第一
		gpMediaPlayer->CurPlayTime = 0;
...
{
	SongInfo* PlayingSongInfo = audio_decoder_get_song_info(audio_decoder);
    ...
    else if((PlayingSongInfo->char_set & 0xF0000000) == CHAR_SET_DIVERSE)
    {
        ...
        type = (PlayingSongInfo->char_set >> 4)  & 0xF;
        if(type == CHAR_SET_UTF_8)
		{
        	StringConvert(PlayingSongInfo->artist,	  MAX_TAG_LEN, PlayingSongInfo->artist,	   MAX_TAG_LEN, UTF8_TO_GBK);
		}
        ...
        type = (PlayingSongInfo->char_set >> 8)  & 0xF;
        if(type == CHAR_SET_UTF_8)
		{
			StringConvert(PlayingSongInfo->album,	  MAX_TAG_LEN, PlayingSongInfo->album,	   MAX_TAG_LEN, UTF8_TO_GBK);
		}
        ...
        type = (PlayingSongInfo->char_set >> 12) & 0xF;
        if(type == CHAR_SET_UTF_8)
		{
			StringConvert(PlayingSongInfo->comment,	  MAX_TAG_LEN, PlayingSongInfo->comment,	   MAX_TAG_LEN, UTF8_TO_GBK);
		}
        ...
        type = (PlayingSongInfo->char_set >> 16) & 0xF;
        if(type == CHAR_SET_UTF_8)
		{
			StringConvert(PlayingSongInfo->genre_str,	  MAX_TAG_LEN, PlayingSongInfo->genre_str,	   MAX_TAG_LEN, UTF8_TO_GBK);
		}
    ...
    if(GetSystemMode() == AppModeFlashPlayBack)
    {
    	APP_DBG("File Size:%d\n", (int)gpMediaPlayer->FileSize);
    	if(PlayingSongInfo->bitrate != 0)
    		APP_DBG("TotalPlayTime:%dms\n", (int)((gpMediaPlayer->FileSize * 8000) / (PlayingSongInfo->bitrate)));
    }
    else
    {
    ...
    //0:Unkown MPEG version, 1:MPEG-1, 2:MPEG-2, 3:MPEG-2.5
    switch(mp3_decoder_get_mpeg_version(audio_decoder))
    {
        case 3:
            APP_DBG("MPEG_2_5");
            break;
    ...
    APP_DBG("\n");
    APP_DBG("Id3Ver:%d\n", (int)mp3_decoder_get_id3_version(audio_decoder));
#endif
    APP_DBG("**************************\n");
    return ;
}
...
bool MediaPlayerOpenSongFile(void)
{
	...
	if(GetSystemMode() == AppModeFlashPlayBack)
	{
		APP_DBG("open flashfile");
		if(gpMediaPlayer->CurFileIndex > gpMediaPlayer->TotalFileSumInDisk
				|| gpMediaPlayer->PlayList[gpMediaPlayer->CurFileIndex -1] >= CFG_PARA_REC_MAX_FILE_NUM
				|| gpMediaPlayer->CurFileIndex == 0)
		{
			return FALSE;
		}
		else
		{
			REC_BLOCK_HEADER Block_Header;
			SPI_Flash_Read(EXTERN_BLOCK_SIZE * (gpMediaPlayer->PlayList[gpMediaPlayer->CurFileIndex - 1]) + REC_BLOCK_ADDR, &Block_Header, sizeof(REC_BLOCK_HEADER));
			gpMediaPlayer->FileSize = Block_Header.FileSize;
			gpMediaPlayer->FileOffset = 0;
		}
	}
	else
#endif
...
bool MediaPlayerDecoderInit(void)
{
	...
	if(GetSystemMode() == AppModeFlashPlayBack)
	{
		mv_mread_callback_set(&gpMediaPlayer->FlashMemHandle,FlashReadFile);

		if(DecoderInit(&gpMediaPlayer->FlashMemHandle, (uint32_t)IO_TYPE_MEMORY, (uint32_t)MP3_DECODER) != RT_SUCCESS)
		{
			APP_DBG("Decoder Init Err\n");
			return FALSE;
		}
...
#endif
	{
		...
		if ((int32_t)SongFileType == FILE_TYPE_MVB)
		{
			mv_fread_call_back_set(DeCRYPT_CRYPT);
		}
		else if ((int32_t)SongFileType == FILE_TYPE_MVC)
		{
			mv_fread_call_back_set(DeCRYPT_CRYPT);
		}
		else if ((int32_t)SongFileType == FILE_TYPE_MVD)
		{
			mv_fread_call_back_set(DeCRYPT_CRYPT);
		}
		else
...
#elif defined(CFG_FUNC_RECORD_FLASH)
    if(GetSystemMode() == AppModeFlashPlayBack && gpMediaPlayer->FileSize)
    {
		APP_DBG("Closeflashfile");

		mv_mread_callback_unset(&gpMediaPlayer->FlashMemHandle);
		gpMediaPlayer->FileSize = 0;
		gpMediaPlayer->FileOffset = 0;
    }
	else
#endif
	...
	switch(GetMediaPlayerState())
	{
		...
		case PLAYER_STATE_IDLE:
			...
			if(!SoftFlagGet(SoftFlagDecoderMask & ~SoftFlagDecoderApp))
			{
				...
				{
					if(SoftFlagGet(SoftFlagDecRecFile))		//判断是否再进行删除录音文件操作下
					{
						MessageContext msgSend;
						msgSend.msgId = MSG_MODE;
						MessageSend(GetMainMessageHandle(), &msgSend);
...
void MediaPlayerFastForward(void)
{
	...
	else //直接调整解码器
	{
		...
		if(gpMediaPlayer->CurPlayTime < 5) return;
...
void MediaPlayerTimeUpdate(void)
{
...
#endif
	{
		...
		if(GetSystemMode() == AppModeFlashPlayBack)
		{
			APP_DBG("^%s%05d.mp3, %02d:%02d ",
					CFG_PARA_FLASH_FILE_NAME,
					(int)gpMediaPlayer->BlockList[gpMediaPlayer->PlayList[gpMediaPlayer->CurFileIndex - 1]],
					(int)(gpMediaPlayer->CurPlayTime ) / 60,
					(int)(gpMediaPlayer->CurPlayTime ) % 60);
		}
		else
...
#ifdef FUNC_SPECIFY_FOLDER_PLAY_EN
		if(gpMediaPlayer->CurPlayMode == PLAY_MODE_REPEAT_FOLDER)	
			SetCurFoldersSongPlayTime(gpMediaPlayer->CurPlayTime);
#endif
#ifdef BP_PART_SAVE_TO_NVM
		if((GetSystemMode() == AppModeUDiskAudioPlay || GetSystemMode() == AppModeCardAudioPlay))
		{
...
bool FlashPlayerInit(void)
{
	...
	memset(gpMediaPlayer->BlockList, 0, sizeof(gpMediaPlayer->BlockList));
	memset(gpMediaPlayer->PlayList, 0, sizeof(gpMediaPlayer->PlayList));
	for(i = CFG_PARA_REC_MAX_FILE_NUM - 1; i >= 0 ; i--)//录音块从前往后，播放队列从后往前排
	{
		SPI_Flash_Read(EXTERN_BLOCK_SIZE * i + REC_BLOCK_ADDR, &Block_Header, sizeof(REC_BLOCK_HEADER));
		{
			if(Block_Header.Index > 0 && Block_Header.Index <= FILE_NAME_MAX && Block_Header.FileSize != FLASH_NONE )
			{
				FileNum++;
				gpMediaPlayer->BlockList[i] = Block_Header.Index;
				gpMediaPlayer->PlayList[FileNum - 1] = i;
				DBG("PlayNum:%d, Block:%d, File:%d\n", FileNum - 1, i, Block_Header.Index);
				for(j = 0; j < FileNum - 1; j++)
				{
					if(gpMediaPlayer->BlockList[gpMediaPlayer->PlayList[j]] < gpMediaPlayer->BlockList[gpMediaPlayer->PlayList[FileNum - 1]])//playerlist 排序，文件号从大到小，存序号
					{
						Backup = gpMediaPlayer->PlayList[j];
						gpMediaPlayer->PlayList[j] = gpMediaPlayer->PlayList[FileNum - 1];
						gpMediaPlayer->PlayList[FileNum - 1] = Backup;
					}
				}
			}
			else if(Block_Header.Index != FLASH_NONE )//登记过录音序号，其他数据出错的自动清理BLOCK
			{
				FlashBlockDel(i);
				DBG("Delfile:%d\n", Block_Header.Index);
			}
	...
	if(FileNum)
	{
		gpMediaPlayer->TotalFileSumInDisk = FileNum;
		gpMediaPlayer->CurFileIndex = 1;
		DBG("Total:%d\n", gpMediaPlayer->TotalFileSumInDisk);
		return TRUE;
	}
...
uint32_t FlashReadFile(void *buffer, uint32_t length) //回放callback
{
	uint32_t Len;
	if(gpMediaPlayer->FileOffset >= gpMediaPlayer->FileSize)
	{
		Len = 0;
	}
	else if(gpMediaPlayer->FileOffset + length > gpMediaPlayer->FileSize)
	{
		Len = gpMediaPlayer->FileOffset + length - gpMediaPlayer->FileSize;
	}
	else
	{
		Len = length;
	}
    if(gpMediaPlayer->PlayList[gpMediaPlayer->CurFileIndex - 1] < CFG_PARA_REC_MAX_FILE_NUM && Len)
    {
    	SPI_Flash_Read(EXTERN_BLOCK_SIZE * (gpMediaPlayer->PlayList[gpMediaPlayer->CurFileIndex - 1]) + gpMediaPlayer->FileOffset, buffer, Len);
    	gpMediaPlayer->FileOffset += Len;
    	return Len;
    }
    return 0;
}
...
*/
	...
	APP_DBG("Del %s.mp3\n",FilePath);
	if(gpMediaPlayer->TotalFileSumInDisk == 0)//无文件退出回放。
	{
		MessageContext		msgSend;
		msgSend.msgId = MSG_MODE;
		MessageSend(GetMainMessageHandle(), &msgSend);
...
#elif defined(CFG_FUNC_RECORD_FLASH)
	int i;
	FlashBlockDel(gpMediaPlayer->PlayList[gpMediaPlayer->CurFileIndex - 1]);
	APP_DBG("Del Flash %s%05d.mp3\n", CFG_PARA_FLASH_FILE_NAME ,gpMediaPlayer->BlockList[gpMediaPlayer->PlayList[gpMediaPlayer->CurFileIndex - 1]]);
	if(gpMediaPlayer->CurFileIndex < gpMediaPlayer->TotalFileSumInDisk)
	{
		for(i = gpMediaPlayer->CurFileIndex - 1; i < gpMediaPlayer->TotalFileSumInDisk - 1; i++)//队列后续迁移
		{
			gpMediaPlayer->PlayList[i] = gpMediaPlayer->PlayList[i + 1];
		}
	}
	else//最早 末尾文件删除
	{
		gpMediaPlayer->CurFileIndex--;//保持最后一文件序号
	}
	gpMediaPlayer->TotalFileSumInDisk--;
...
#endif	//defined(CFG_FUNC_RECORD_FLASH)
...
#endif //defined(FUNC_BROWSER_TREE_EN)||defined(FUNC_SPECIFY_FOLDER_PLAY_EN)
...
#endif //FUNC_BROWSER_PARALLEL_EN
#endif//defined(CFG_APP_USB_PLAY_MODE_EN) || defined(CFG_APP_CARD_PLAY_MODE_EN) || defined(CFG_FUNC_RECORDER_EN)
