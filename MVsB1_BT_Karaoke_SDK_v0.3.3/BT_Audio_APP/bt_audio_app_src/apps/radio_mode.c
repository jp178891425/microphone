...
//	//note Soure0.和sink0已经在main app中配置，不要随意配置
	...
	RadioPlayCt->runflag = 0;
	RadioPlayCt->umuteflag = 0;
	AudioCoreSourceMute(APP_SOURCE_NUM, TRUE, TRUE);
...
static void RadioPlayModeStarting(uint16_t msgId)
{
...
#endif
	{
		...
		{
			RadioPlayCt->runflag = 1;
			RadioPlayCt->umuteflag = 0;
			TimeOutSet(&RadioPlayCt->umuteTime, 200);
		}
#else
		if(!SoftFlagGet(SoftFlagRemindMask))
		{
			...
			#endif
			{
				if(!RemindSoundServiceItemRequest(SOUND_REMIND_FMMODE, REMIND_ATTR_NORMAL)) //插播提示音
				{
					RadioPlayCt->runflag = 1;
					RadioPlayCt->umuteflag = 0;
					TimeOutSet(&RadioPlayCt->umuteTime, 200);
...
static void RadioPlayRunning(uint16_t msgId)
{
	...
	if(RadioPlayCt->umuteflag == 0)
	{
		if(RadioPlayCt->runflag == 1)
		{
			if(IsTimeOut(&RadioPlayCt->umuteTime))
			{
				AudioCoreSourceUnmute(APP_SOURCE_NUM, TRUE, TRUE);
			#ifdef BT_TWS_SUPPORT
	...
	switch(msgId)
	{
		...
		case MSG_PLAY_PAUSE:
			...
			{
				extern void EncoderServicePause(void);
				EncoderServicePause();
				break;
			}
#endif			
...
bool RadioPlaySourceIsNeedCurrentUnmute(void)
{
	if(RadioPlayCt == NULL)
		return 0;
	if(RadioPlayCt->umuteflag == 1)
		return 1;
	else
		return 0;
}

void RadioPlaySourceUnmuteSet(void)
{
	if(RadioPlayCt == NULL)
		return;
	RadioPlayCt->runflag = 1;
