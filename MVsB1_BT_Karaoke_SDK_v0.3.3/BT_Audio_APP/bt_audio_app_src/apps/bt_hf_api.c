...
{
	if(InLen == 0 || BtHfCt == NULL || BtHfCt->Sink2ScoFifoMutex == NULL)
		return 0;
...
{
	if(OutLen == 0 || BtHfCt == NULL || BtHfCt->Sink2ScoFifoMutex == NULL)
		return;
...
{
	BtHfCt->msbcRingBuf = (uint8_t *)osPortMalloc(MSBC_DECODER_INPUT_SIZE);
	if(BtHfCt->msbcRingBuf == NULL)
		return -1;
	memset(BtHfCt->msbcRingBuf, 0, MSBC_DECODER_INPUT_SIZE);
...
int32_t BtHf_MsbcDecoderStart(void)
{
	...
	DecoderPlay();
	audio_decoder_get_song_info(audio_decoder);
...
int16_t BtHf_SaveScoData(uint8_t* data, uint16_t len)
{
	...
	if(BtHfCt->codecType == HFP_AUDIO_DATA_PCM)
	{
		...
		else if(len == 60)
		{
			...
			//send data(60Bytes)
			if(BtHf_SendScoBufLenGet() >= 60)
			{
				memset(BtHfCt->scoSendBuf,0,60);
				BtHf_SendScoBufGet(BtHfCt->scoSendBuf, 60);
				HfpSendScoData(BtHfCt->scoSendBuf, 60);
			}

			BtHfCt->BtHfScoBufBkCnt++;
			if(BtHfCt->BtHfScoBufBkCnt%2)
			{
				memcpy(&BtHfCt->BtHfScoBufBk[0], data, 60);
				return 0;
			}
			else
			{
				memcpy(&BtHfCt->BtHfScoBufBk[60], data, 60);
			}
			...
			{
				cvsd_plc_good_frame(BtHfCt->cvsdPlcState, (int16_t *)BtHfCt->BtHfScoBufBk, (int16_t *)BtHfCt->ScoInputFifo);
			}
			
			BtHfCt->ScoInputLen = 120;
