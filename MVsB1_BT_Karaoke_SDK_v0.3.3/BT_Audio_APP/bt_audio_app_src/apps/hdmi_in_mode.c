...
static void HdmiInPlayModeStarting(uint16_t msgId)
{
...
#endif
	{
		...
		if(!SoftFlagGet(SoftFlagRemindMask))
		{
			...
			#endif
			{
				if(!RemindSoundServiceItemRequest(SOUND_REMIND_HDMIMODE, REMIND_ATTR_NORMAL)) //插播提示音
...
static void HdmiInPlayModeStopping(uint16_t msgId)
{
		...
		)
	{
		...
		if(gHdmiCt->hdmi_tv_inf.tv_type != TV_SONY_047C)
		{
			if(gCecInitDef)
			{
				if((gHdmiCt->hdmi_tv_inf.tv_type != TV_POLARIOD_010B)&&(gHdmiCt->hdmi_tv_inf.tv_type != TV_TCL_2009))
				{
...
static void HdmiARCScan(void)
{
	...
	while(cnt--)
	{
        ...
        SPDIF_SPDIFDatatoAudioData((int32_t *)pcmBuf, (MAX_FRAME_SAMPLES - hdmiInPlayCt->pcmRemainLen) * 8, (int32_t *)pcmBuf, SPDIF_WORDLTH_16BIT, &hdmiInPlayCt->AudioInfo);
