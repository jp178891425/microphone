...
static const uint8_t DmaChannelMap[29] = {
	...
	#endif
	#if defined(CFG_FUNC_I2S_MIX_MODE) || defined(CFG_RES_AUDIO_SPDIFOUT_EN)
...
uint32_t hfMode3WayActiveCallState = 0;
...
void SetBtHfModeEnterFlag(uint32_t flag)
{
	BtHfModeEnterFlag = flag;
}
uint32_t getBtHfModeEnterFlag(void)
{
...
//	memset(BtHfCt->Source1Buf_ScoData, 0, SampleLen * 2 * 2);
	...
	BtHfCt->SourceDecoder = (int16_t *)osPortMallocFromEnd(SampleLen * 2 * 2 * 2);//One Frame
	if(BtHfCt->SourceDecoder == NULL)
	{
		return FALSE;
	}
	memset(BtHfCt->SourceDecoder, 0, SampleLen * 2 * 2 * 2);//2K
#endif
	return TRUE;
}
...
//CVSD 转采样配置 8K->16K       16K->8K
	...
	//cvsd plc config
	BtHfCt->cvsdPlcState = (CVSD_PLC_State*)osPortMalloc(sizeof(CVSD_PLC_State));
...
{
	{
		if(BtHfCt->hfModeAudioResInitOk)
		{
			if((!BtHfCt->CvsdInitFlag)&&(!BtHfCt->MsbcInitFlag))
			{
				...
				//adc
				AudioADC1ParamsSet(BtHfCt->SampleRate);
...
static bool BtHfInit(MessageHandle parentMsgHandle)
{
	...
	uint16_t Len;
	Len = mainAppCt.SamplesPreFrame * 2 * 2 *2;
	DMA_ChannelDisable(PERIPHERAL_ID_AUDIO_DAC0_TX);
	DMA_CircularConfig(PERIPHERAL_ID_AUDIO_DAC0_TX, Len/2, (void*)mainAppCt.DACFIFO, Len);
	...
	BtHfCt->ScoInputFifo = (uint8_t*)osPortMalloc(SCO_INPUT_FIFO_SIZE);
	...
	BtHfCt->ScoOutputFifo = (int16_t*)osPortMalloc(SCO_OUTPUT_FIFO_SIZE);
...
#endif
	gCtrlVars.sample_rate = CFG_BTHF_PARA_SAMPLE_RATE;
//DAC0/DACX
		...
		if(BtHfCt->RemindCnt>=100)
		{
			...
			if(GetHfpState() == BT_HFP_STATE_INCOMING)
			{
				while((BtHfCt->PhoneNumberPos <= BtHfCt->PhoneNumberLen))
				{
					...
					if (BtHfCt->PhoneNumberPos < BtHfCt->PhoneNumberLen)
					{
						if(RemindSoundServiceItemRequest(SoundItem, REMIND_ATTR_ORDER))
						{
					...
					else if(BtHfCt->PhoneNumberPos == BtHfCt->PhoneNumberLen)
					{
						if (RemindSoundServiceItemRequest(SOUND_REMIND_CALLRING, REMIND_ATTR_ORDER))
						{
							BtHfCt->PhoneNumberPos++;
						}
						else
						{
							DBG("CALLRING Phone Number play error!\n");
							BtHfCt->PhoneNumberPos = 0;
							break;
						}	
					}
...
#ifdef CFG_FUNC_I2S_MIX_MODE
	AudioI2S_Release();
	AudioI2S_ResMalloc(gCtrlVars.SamplesPerFrame);
	AudioI2S_ConfigInit();
#endif
...
//------------------------------------------------//
			...
			case MSG_BT_HF_MODE_REMIND_PLAY:
					...
					APP_DBG("MSG_BT_HF_MODE_REMIND_PLAY= phone number len:%d\n", BtHfCt->PhoneNumberLen);
#endif
				break;
#endif
//-------------------------------//
			...
			case MSG_BT_HF_MODE_REMIND_PLAY:
			   ...
			   if( (GetHfpState() == BT_HFP_STATE_INCOMING) \
				...
				&& (!SoftFlagGet(SoftFlagNoRemind))\
				&& ((BtHfCt->CvsdInitFlag) || (BtHfCt->MsbcInitFlag)) )
#else					
			   if((GetHfpState() == BT_HFP_STATE_INCOMING) \
			  && ((BtHfCt->CvsdInitFlag) || (BtHfCt->MsbcInitFlag)) )
#endif
				{
                    ...
                    #ifdef CFG_FUNC_REMIND_SOUND_EN
					if(!RemindSoundServiceItemRequest(SOUND_REMIND_CALLRING, REMIND_ATTR_NORMAL))
					#else
					#ifdef CFG_FUNC_REMIND_MIX_MODE
					if(!RemindMixServiceItemRequest(SOUND_REMIND_CALLRING, FALSE))
					#endif//end of CFG_FUNC_REMIND_MIX_MODE
					#endif//end of CFG_FUNC_REMIND_SOUND_EN
						{
							APP_DBG("Play Local Ring Error!!!\n");
						}
					}
#elif (CFG_BT_RING_MODE == RING_NUMBER_REMIND)
					BtHfCt->PhoneNumberLen = GetBtCallInPhoneNumber(BtHfCt->PhoneNumber);
					if(BtHfCt->PhoneNumberLen)
						BtHfCt->PhoneNumberRemindStart = 1;
...
/*#ifdef CFG_FUNC_FREQ_ADJUST
			...
			case MSG_BT_HF_CALL_REJECT:
				switch(GetHfpState())
				{
					...
					case BT_HFP_STATE_3WAY_INCOMING_CALL:
						//挂断第三方来电，继续保持原有通话
						HfpCallHold(HF_HOLD_RELEASE_HELD_CALLS);
						break;
					case BT_HFP_STATE_3WAY_OUTGOING_CALL:
						HfpHangup();
						SetHfpState(BT_HFP_STATE_ACTIVE);
		...
		if(BtHfCt->BtHfTimeUpdate)
		{
			...
			BtHfCt->BtHfTimeUpdate = 0;
			APP_DBG("通话中：%02d:%02d:%02d:  %02d\n", hour, minute, second, GetHfpState());
			//在三方通话状态下,每隔1s获取通话状态1次
...
#ifndef CFG_FUNC_MIC_KARAOKE_EN
	uint16_t Len;
	Len = mainAppCt.SamplesPreFrame * 2 * 2 *2 * 4;
	DMA_ChannelDisable(PERIPHERAL_ID_AUDIO_DAC0_TX);
	DMA_CircularConfig(PERIPHERAL_ID_AUDIO_DAC0_TX, Len/2, (void*)mainAppCt.DACFIFO, Len);
	DMA_ChannelEnable(PERIPHERAL_ID_AUDIO_DAC0_TX);
#endif
...
void EnterBtHfMode(void)
{
	...
	if((!btManager.btLinkState)||(SoftFlagGet(SoftFlagWaitBtRemindEnd)))
	{
		...
		DBG("btManager.btLinkState = %u    SoftFlagGet(SoftFlagWaitBtRemindEnd) = %u\n", btManager.btLinkState, SoftFlagGet(SoftFlagWaitBtRemindEnd));
		return;
	}
	...
	if((GetSystemMode() != AppModeBtHfPlay)&&(!BtHfModeEnterFlag))
	{
		...
		//通话模式进入标志置起来
		SetBtHfModeEnterFlag(1);
		BtHfModeEixtList = 0;
	}
	else
	{
		DBG("GetSystemMode() = %lu   BtHfModeEnterFlag = %lu\n", GetSystemMode(), BtHfModeEnterFlag);
	}
}
