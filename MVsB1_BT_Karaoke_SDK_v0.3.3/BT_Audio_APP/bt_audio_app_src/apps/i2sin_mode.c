...
static void I2SInPlayModeStarting(uint16_t msgId)
{
...
#endif
	{
		...
		if(!SoftFlagGet(SoftFlagRemindMask))
		{
			...
			#endif
			{
				if(!RemindSoundServiceItemRequest(SOUND_REMIND_I2SMODE, REMIND_ATTR_NORMAL))//插播提示音：模式I2S启动
				{
...
#endif
		vTaskDelay(10);
		AudioCoreSourceEnable(I2SIN_SOURCE_NUM);//避免tws pop音，先播提示音 再使能
	}
}
...
bool I2SInPlayKill(void)
{
	...
	DMA_ChannelDisable(PERIPHERAL_ID_I2S0_RX);//USB 声卡模式切换死机问题(内存泄露问题)
	...
	DMA_ChannelDisable(PERIPHERAL_ID_I2S1_RX); // USB 声卡模式切换死机问题(内存泄露问题)
#ifndef CFG_RES_AUDIO_I2SOUT_EN
	I2S_ModuleDisable(I2S1_MODULE);
	DMA_ChannelDisable(PERIPHERAL_ID_I2S1_RX);
#endif	
