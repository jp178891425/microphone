...
//	//note Soure0.和sink0已经在main app中配置，不要随意配置
	...
	LineInPlayCt->runflag = 0;
	LineInPlayCt->umuteflag = 0;
...
static void LineInPlayModeStarting(uint16_t msgId)
{
...
#endif
	{
...
#ifndef CFG_FUNC_REMIND_SOUND_EN			
		{
			...
			LineInPlayCt->runflag = 1;
			LineInPlayCt->umuteflag = 0;
			TimeOutSet(&LineInPlayCt->umuteTime, 200);
            #ifdef CFG_FUNC_REMIND_MIX_MODE
			RemindMixServiceItemRequest(SOUND_REMIND_XIANLUMO,0);
            #endif
		}
#else
		if(!SoftFlagGet(SoftFlagRemindMask))
		{
			...
			#endif
			{
				if(!RemindSoundServiceItemRequest(SOUND_REMIND_XIANLUMO,REMIND_ATTR_NORMAL)) //插播提示音：模式line启动
				{
...
#endif
		vTaskDelay(10);
		AudioCoreSourceEnable(LINEIN_SOURCE_NUM);//避免tws pop音，先播提示音 再使能
	}
}
...
static void LineInPlayRunning(uint16_t msgId)
{
	...
	{
		if(LineInPlayCt->umuteflag == 0)
		{
			if(LineInPlayCt->runflag == 1)
			{
				#if (defined(BT_TWS_SUPPORT) && CFG_EFFECT_MUSIC_MASTER == 1)
				if(IsTimeOut(&LineInPlayCt->umuteTime))
				#endif
				{
					AudioCoreSourceUnmute(APP_SOURCE_NUM, TRUE, TRUE);
					#ifdef BT_TWS_SUPPORT
		...
		switch(msgId)
		{
			...
			case MSG_REC:
				...
				if(ResourceValue(AppResourceFlashFs))
#elif defined(CFG_FUNC_RECORD_FLASH)
				if(ResourceValue(AppResourceFlash))
#endif
				{
					if(LineInPlayCt->RecorderSync == TaskStateNone )
					{
...
bool LineInSourceIsNeedCurrentUnmute(void)
{
	if(LineInPlayCt == NULL)
		return 0;
	if(LineInPlayCt->umuteflag == 1)
		return 1;
	else
		return 0;
}

void LineInSourceUnmuteSet(void)
{
	if(LineInPlayCt == NULL)
		return;
