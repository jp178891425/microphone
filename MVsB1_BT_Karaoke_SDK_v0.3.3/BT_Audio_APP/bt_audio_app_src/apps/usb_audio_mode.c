...
extern volatile uint8_t Src_over_flag;

void UsbDevicePlayResRelease(void)
{
	uint16_t Src_over_cnt = 0;
...
//pc->chip
	...
	if(UsbAudioSpeaker.Resampler == NULL)
	{
		APP_DBG("UsbAudioSpeaker.Resampler memory error\n");
		return FALSE;
	}
#endif  ///end of CFG_RES_AUDIO_USB_SRC_EN
	UsbAudioSpeaker.SRCOutBuf = (int16_t*)osPortMallocFromEnd(USB_AUDIO_SRC_BUF_LEN);
	if(UsbAudioSpeaker.SRCOutBuf == NULL)
	{
		APP_DBG("UsbAudioSpeaker.SRCOutBuf memory error\n");
		return FALSE;
	}


	//Speaker FIFO
	UsbAudioSpeaker.PCMBuffer = (int16_t*)osPortMallocFromEnd(SampleLen * 16);
	if(UsbAudioSpeaker.PCMBuffer == NULL)
	{
	...
	UsbAudioMic.SRCOutBuf = (int16_t*)osPortMallocFromEnd(USB_AUDIO_SRC_BUF_LEN);
	if(UsbAudioMic.SRCOutBuf == NULL)
	{
	...
	//MIC FIFO
	UsbAudioMic.PCMBuffer = (int16_t*)osPortMallocFromEnd(SampleLen * 16);
	if(UsbAudioMic.PCMBuffer == NULL)
	{
...
//Task & App Config
	...
	UsbDevicePlayCt->AudioCoreUsb->AudioSource[USB_AUDIO_SOURCE_NUM].IsSreamData = TRUE;
#else
	...
	UsbDevicePlayCt->AudioCoreUsb->AudioSink[USB_AUDIO_SINK_NUM].PcmOutBuf = (int16_t *)UsbDevicePlayCt->Sink1Buf_USBAduio;
	UsbDevicePlayCt->AudioCoreUsb->AudioSink[USB_AUDIO_SINK_NUM].PcmFormat = 2;
	...
	{
		DBG("@!@@!USB phone mode\n");
		uint16_t DACAudioSpeakerDataGet(void *Buffer, uint16_t Len);
		uint16_t DACAudioSpeakerDataLenGet(void);
		gCtrlVars.sample_rate = CFG_BTHF_PARA_SAMPLE_RATE;

		//AEC
		Audio_AECInit(&UsbDevicePlayCt->AecContext);
		Audio_AECReset(&UsbDevicePlayCt->AecContext, USB_DEFAULT_DELAY_BLK);

		AudioADC1ParamsSet(16000);
...
static void UsbDevicePlayModeStarting(uint16_t msgId)
{
...
#endif
	{
		...
		if(!SoftFlagGet(SoftFlagRemindMask))
		{
			...
			#endif
			{
				if(!RemindSoundServiceItemRequest(SOUND_REMIND_SHENGKAM, REMIND_ATTR_ORDER)) //插播提示音:模式启动
				{
...
//#endif
	...
	if(GetSystemMode() == AppModeUsbPhone)
	{
		mainAppCt.EffectMode = EFFECT_MODE_USB_AEC;
	...
	while(1)
	{
		...
		switch(msgRecv.msgId)
		{
			...
			case MSG_REMIND_SOUND_PLAY_START:
				APP_DBG("usb device: MSG_REMIND_SOUND_PLAY_START: \n");
				break;
					
			case MSG_REMIND_SOUND_PLAY_DONE://提示音播放结束
			case MSG_REMIND_SOUND_PLAY_REQUEST_FAIL:
				APP_DBG("usb device: MSG_REMIND_SOUND_PLAY_DONE: \n");
				AudioCoreSourceUnmute(APP_SOURCE_NUM, TRUE, TRUE);
				#ifdef BT_TWS_SUPPORT
					AudioCoreSourceUnmute(TWS_SOURCE_NUM, TRUE, TRUE);
				#endif
...
#ifndef	CFG_APP_USB_PHONE_MODE_EN
# if( (CFG_PARA_USB_MODE == READER) || (CFG_PARA_USB_MODE == AUDIO_READER) || (CFG_PARA_USB_MODE == MIC_READER) || (CFG_PARA_USB_MODE == AUDIO_MIC_READER) )
...
static void UsbDevicePlayRunning(uint16_t msgId)
{
	switch(msgId)
	{
		...
		case MSG_TASK_POWERDOWN:
			APP_DBG("UsbDevice:PowerDown MSG, Please breakpoint\n");
			SystemPowerDown();
			break;
#endif
...
//	uint16_t sLen,k;
	...
	for(i = 0; i < temp0; i++)
	{
		...
		{
			SRCDoneLen = resampler_polyphase_apply(UsbAudioMic.Resampler, audioSinkAdjustCt->SraPcmOutnBuf, UsbAudioMic.SRCOutBuf, SRADoneLen);
			if(SRCDoneLen < 0) SRCDoneLen = 0;
			MCUCircular_PutData(&UsbAudioMic.CircularBuf, UsbAudioMic.SRCOutBuf, SRCDoneLen * 4);
		}
		else
