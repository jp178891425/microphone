...
int16_t usb_iso_in_buf[128*2+64];
int16_t usb_iso_out_buf[128*2+64];
uint8_t    usb_busy = 0;//解决发送中断命令
#endif
	...
	if(SampleRate != CFG_PARA_SAMPLE_RATE)
	{
		if(UsbAudioMic.Resampler)
		{
		  UsbAudioMic.AudioSampleRateFlag = TRUE;
		   //resampler_init(UsbAudioMic.Resampler, 2,CFG_PARA_SAMPLE_RATE, SampleRate, 0, 0);
			if (SampleRate == 48000)//44k->SampleRate(48k)
			{
			  resampler_polyphase_init(UsbAudioMic.Resampler, 2, RESAMPLER_POLYPHASE_SRC_RATIO_160_147);
			}
//			#endif
//
			if (SampleRate == 44100)//48k->SampleRate(44k)
			{
...
{
#if ((defined CFG_RES_AUDIO_USB_IN_EN)&& (defined CFG_RES_AUDIO_USB_IN_SRC_EN))
	//if(SampleRate == 48000)
	if(SampleRate != CFG_PARA_SAMPLE_RATE)
	{
		if(UsbAudioSpeaker.Resampler)
		{
		  ...
		  //resampler_init(UsbAudioSpeaker.Resampler, 2, SampleRate, CFG_PARA_SAMPLE_RATE, 0, 0);
			if (SampleRate == 44100)//44k->SampleRate(48k)
			{
			  resampler_polyphase_init(UsbAudioSpeaker.Resampler, 2, RESAMPLER_POLYPHASE_SRC_RATIO_160_147);
			}
//			#endif
//
			if (SampleRate == 48000)//48k->SampleRate(44k)
			{
			  resampler_polyphase_init(UsbAudioSpeaker.Resampler, 2, RESAMPLER_POLYPHASE_SRC_RATIO_147_160);
			}
//			#endif
			 //resampler_polyphase_init(UsbAudioSpeaker.Resampler, 2, Get_Resampler_Polyphase(44100));
	...
	DBG("USB Speaker SampleRate Set:%ld\n",SampleRate);

}
#endif // end of #ifdef CFG_FUNC_USB_MIX_MODE
//转采样直接在中断中处理，转采样时间大约是180us。
//注意一下需要4字节对齐
volatile uint8_t Src_over_flag = 0;
void OnDeviceAudioRcvIsoPacket(void)
{
	...
	uint32_t channel = UsbAudioSpeaker.Channel;
		...
		if(UsbAudioSpeaker.AudioSampleRate == 0)
		{
			...
			else if(Len == 64)
			{
				channel = 2;
				sample = Len/4;
				UsbAudioSpeaker.AudioSampleRate = 16000;
			}
			else if(Len == 32)
			{
				channel = 1;
		...
		)
		{
			...
			SRCDoneLen = resampler_polyphase_apply(UsbAudioSpeaker.Resampler, iso_buf, usb_iso_out_buf, sample);
			if(SRCDoneLen < 0) SRCDoneLen = 0;
			sample =  sample&0x000000FF;
			sample = SRCDoneLen&0xFF;
			memcpy(iso_buf,usb_iso_out_buf,SRCDoneLen*4);
			//DBG("*");
		}
		...
		if(UsbAudioSpeaker.PCMBuffer ==NULL)
		{
		 ...
		 Len = usb_iso_speaker((uint8_t *)iso_buf, (uint8_t *)usb_iso_in_buf, sample*2*channel);
		 MCUCircular_PutData(&UsbAudioSpeaker.CircularBuf, (uint8_t*)usb_iso_in_buf, Len);
	
#endif //end of #ifdef CFG_RES_AUDIO_USB_IN_EN
	...
	if(MCUCircular_GetDataLen(&UsbAudioSpeaker.CircularBuf)/4 >= 128 && MCUCircular_GetSpaceLen(&UsbAudioSpeaker.CircularBufSRA)/4 > 128 + 1)
	{
...
void OnDeviceAudioSendIsoPacket(void)
{
	...
	else if(UsbAudioMic.AudioSampleRate == 16000)
	{
		RealLen = 16;
	}
	else
