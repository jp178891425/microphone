...
static bool MediaPlayInit(MessageHandle parentMsgHandle)
{
	...
	{
		MediaPlayCt->Device = DEV_ID_FLASH;
	}
#else
	...
	{
		gpMediaPlayer = (MEDIA_PLAYER*)osPortMalloc(sizeof(MEDIA_PLAYER));
		if(gpMediaPlayer == NULL)
		{
...
static void MediaPlayModeCreating(uint16_t msgId)
{
	...
	{
		APP_DBG("Media:Decoder service created\n");
...
static void MediaPlayModeStarting(uint16_t msgId)
{
	...
	{
		APP_DBG("Media:Decoder service started\n");
		...
		if(GetSystemMode() == AppModeCardAudioPlay)
		{
			if(!SoftFlagGet(SoftFlagRemindMask))
			{
				...
				#endif
				{
					MediaPlayCt->ModeRemindWait = RemindSoundServiceItemRequest(SOUND_REMIND_CARDMODE, REMIND_ATTR_NORMAL);
				}
			}
			else
			{
		...
		else if(GetSystemMode() == AppModeUDiskAudioPlay)
		{
			if(!SoftFlagGet(SoftFlagRemindMask))
			{
				...
				#endif
				{
					MediaPlayCt->ModeRemindWait = RemindSoundServiceItemRequest(SOUND_REMIND_UPANMODE, REMIND_ATTR_NORMAL);
				}
			}
			else
			{
		...
		#ifdef CFG_FUNC_RECORDER_EN
		else if((GetSystemMode() == AppModeUDiskPlayBack) || (GetSystemMode() == AppModeCardPlayBack))
		{
			if(!SoftFlagGet(SoftFlagRemindMask))
			{
				#ifdef BT_TWS_SUPPORT
				...
				#endif
				{
					MediaPlayCt->ModeRemindWait = RemindSoundServiceItemRequest(SOUND_REMIND_RECHUIFA, REMIND_ATTR_NORMAL);
				}
			}
			else
			{
		...
		if(GetSystemMode() == AppModeCardAudioPlay)
		{
			if(!SoftFlagGet(SoftFlagRemindMask))
			{
				...
				{
					SoftFlagDeregister(SoftFlagTwsDisconnectRemind);
					RemindMixServiceItemRequest(SOUND_REMIND_TWS_DISC, 0);
				}
				else
				#endif
				{
					RemindMixServiceItemRequest(SOUND_REMIND_CARDMODE, 0);
				}
			}
			else
			{
				...
				{
					SoftFlagDeregister(SoftFlagDiscDelayMask);
					RemindMixServiceItemRequest(SOUND_REMIND_DISCONNE, 0);
				}
			}
		}
		else if(GetSystemMode() == AppModeUDiskAudioPlay)
		{
			if(!SoftFlagGet(SoftFlagRemindMask))
			{
				...
				#endif
				{
					RemindMixServiceItemRequest(SOUND_REMIND_UPANMODE, 0);
				}
			}
			else
			{
		...
		else if((GetSystemMode() == AppModeUDiskPlayBack) || (GetSystemMode() == AppModeCardPlayBack))
		{
			if(!SoftFlagGet(SoftFlagRemindMask))
			{
				...
				#endif
				{
					RemindMixServiceItemRequest(SOUND_REMIND_RECHUIFA, 0);
				}
			}
			else
			{
		...
		#endif
      #endif //end of #if defined(CFG_FUNC_REMIND_MIX_MODE)
#endif
		{
			MediaPlayerStart();
		}
...
static void MediaPlayModeStopping(uint16_t msgId)
{
	...
	&& MediaPlayCt->state == TaskStateStopping)
	{
		APP_DBG("Media:Decoder service Stopped\n");
		//Set para
	...
	while(1)
	{
		...
		switch(msgRecv.msgId)//此段代码禁止新增提示音
		{
...
#ifdef CFG_FUNC_REMIND_SOUND_EN
					if(RemindSoundServiceItemRequest(SOUND_REMIND_LUYIN, REMIND_ATTR_NORMAL))
					{
			...
			case MSG_REMIND_SOUND_NEED_DECODER:
				APP_DBG("Media:Stop for remind\n");
				if(MediaPlayCt->ModeRemindWait)
				{
					APP_DBG("Media:first remind, decoder switch out\n");
			...
			if(MediaPlayCt->ModeRemindWait)
				{
					APP_DBG("Media:first remind, decoder switch in\n");
				...
				else if(MediaPlayCt->state == TaskStateRunning)
				{
					...
					{
						APP_DBG("Media:decoder switch out\n");
						AudioCoreSourceDisable(MediaPlayCt->SourceNum);
						SoftFlagRegister(SoftFlagDecoderRemind);//出让解码器
						RemindSoundServicePlay();
					}
		...
		//				AudioCoreSourceUnmute(MediaPlayCt->SourceNum, TRUE, TRUE);
...
#ifdef CFG_FUNC_REMIND_SOUND_EN
					MediaPlayCt->ModeRemindWait = FALSE;
					RemindSoundServiceItemRequest(SOUND_REMIND_STOPREC, REMIND_ATTR_NORMAL);
					osTaskDelay(350);
#else
                    #ifdef CFG_FUNC_REMIND_MIX_MODE
					RemindMixServiceItemRequest(SOUND_REMIND_STOPREC, 0);
					osTaskDelay(350);
                    #endif
#endif
			...
			case MSG_REMIND_SOUND_PLAY_START:
				APP_DBG("media play: MSG_REMIND_SOUND_PLAY_START: \n");
				break;
					
			case MSG_REMIND_SOUND_PLAY_DONE://提示音播放结束
				APP_DBG("media play: MSG_REMIND_SOUND_PLAY_DONE: \n");
#ifdef CFG_FUNC_RECORDER_EN	
#ifdef	CFG_FUNC_REMIND_SOUND_EN
				SoftFlagDeregister(SoftFlagMediaPlayRecRemind);
#endif
#endif
#ifdef FUNC_SPECIFY_FOLDER_PLAY_EN        //上下曲不做处理，其他提示音结束则从文件夹读取歌曲    
                if(!gpMediaPlayer->PreNextPlayFlag)
                {
                    gpMediaPlayer->StorySelectPlayFlag=STORY_PLAY_CURRENT;
                }
                else
                {
                	gpMediaPlayer->PreNextPlayFlag = FALSE;
                }
#endif
...
#elif defined(CFG_FUNC_RECORD_FLASHFS)	|| defined(CFG_FUNC_RECORD_FLASH)
				if(MediaPlayCt->RecorderSync == TaskStateNone)
				 {
					SetMediaPlayerState(PLAYER_STATE_STOP);
					MediaPlayerDecoderStop();
...
static void MediaPlayRunning(uint16_t msgId)
{
	...
	switch(msgId)
	{
		...
		case MSG_TASK_POWERDOWN:
			APP_DBG("Media:PowerDown, Please breakpoint\n");
			SystemPowerDown();
			break;
#endif
	...
	*/	
		...
		//case MSG_DECODER_SERVICE_FF_END_SONG:
			if(GetDecoderState() == DecoderStatePause)//确保播放结束 ，依旧暂停态，减少stop消息交叉
			{
				...
				if(GetSystemMode() == AppModeUDiskAudioPlay)
				{
					osTaskDelay(10);				//防止U盘拔出标志未来得及置位
					if(UDiskRemovedFlagGet() == TRUE)
					{
						APP_DBG("Media:UDisk Removed play end\n");
						break;
					}
				}
#endif
				...
				if(CmdErrCnt >= 3)//临时处理U盘异常但是U盘插着的情况
				{
					CmdErrCnt = 0;
					APP_DBG("cmd error >= 3\n");
					extern bool IsMediaPlugOut(void);
					if(IsMediaPlugOut())
					{
						MessageContext      msgSend;
						msgSend.msgId = MSG_MODE;
...
#ifdef CFG_FUNC_REMIND_SOUND_EN
			//RemindSound request
			//RemindSoundServiceItemRequest(SOUND_REMIND_XIAYISOU, REMIND_ATTR_NORMAL);
#else
            #ifdef CFG_FUNC_REMIND_MIX_MODE
			RemindMixServiceItemRequest(SOUND_REMIND_XIAYISOU, 0);
            #endif
#endif
...
#ifdef CFG_FUNC_REMIND_SOUND_EN
			//RemindSound request
			RemindSoundServiceItemRequest(SOUND_REMIND_SHANGYIS, REMIND_ATTR_NORMAL);
#else
            #ifdef CFG_FUNC_REMIND_MIX_MODE
			RemindMixServiceItemRequest(SOUND_REMIND_SHANGYIS, 0);
            #endif
#endif
#ifdef CFG_FUNC_TSM_EN
			gCtrlVars.tsm_unit.tsm_en =  0;
		...
		case MSG_TSM_UP:
			...
			gCtrlVars.tsm_unit.speed_ratio += 4096;

			if(gCtrlVars.tsm_unit.speed_ratio>65536)
			{
				gCtrlVars.tsm_unit.speed_ratio = 65536;//max
			}
            AudioEffectTsmConfigure(gCtrlVars.tsm_unit.speed_ratio);

			if(!gCtrlVars.tsm_unit.tsm_en)
			{
				vTaskDelay(20);
				gCtrlVars.tsm_unit.tsm_en =  1;
			}
			break;
		case MSG_TSM_DN:
			DBG("Media:TSM-\n");
			...
			if(gCtrlVars.tsm_unit.speed_ratio <16384)
			{
				gCtrlVars.tsm_unit.speed_ratio = 16384;//min
			}
            AudioEffectTsmConfigure(gCtrlVars.tsm_unit.speed_ratio);
