...
//  AppMode						Priority(0:Lowest)	ModeResourceMask	
...
#ifdef BT_TWS_SUPPORT
	{AppModeTwsSlavePlay,		3,					AppResourceTwsSlave	},
#endif
#endif
	...
	{AppModeFlashPlayBack,		0,					AppResourceFlash	},
#endif
...
#endif
#ifdef BT_TWS_SUPPORT
int32_t TwsSlaveStateSet(ModeStateAct StateAct)
{
	int32_t ret = 0;
	switch(StateAct)
	{
		case ModeCreate:
			ret = TwsSlavePlayCreate(GetMainMessageHandle());
			break;
		
		case ModeStart:
			ret = TwsSlavePlayStart();
			break;

		case ModeStop:
			ret = TwsSlavePlayStop();
			break;

		case ModeKill:
			ret = TwsSlavePlayKill();
			break;
...
#endif	
#ifdef BT_TWS_SUPPORT
		case AppModeTwsSlavePlay:
			ret = TwsSlaveStateSet(StateAct);
			break;
#endif
#endif
#ifdef CFG_APP_HDMIIN_MODE_EN
...
#ifdef BT_TWS_SUPPORT
		case MSG_TWS_SLAVE_MODE_CREATED:
#endif
...
#ifdef BT_TWS_SUPPORT
		case MSG_TWS_SLAVE_MODE_STARTED:
#endif
...
#ifdef BT_TWS_SUPPORT
		case MSG_TWS_SLAVE_MODE_STOPPED:
#endif
