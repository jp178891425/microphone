...
static void RestPlayModeStarting(uint16_t msgId)
{
...
#endif
	{
...
#endif
		{
			...
			RemindSoundServiceItemRequest(SOUND_REMIND_GUANJI, REMIND_ATTR_NORMAL);//SOUND_REMIND_STANDBYM
#else
...
//					&& RemindSoundServiceItemRequest(SOUND_REMIND_KAIJI, REMIND_ATTR_NORMAL))
//			{
...
//#endif
	...
	if(mainAppCt.AlarmRemindStart)
	{
		if(!RemindSoundServiceIsPlaying())
		{
			#ifdef CFG_FUNC_REMIND_SOUND_EN
			RemindSoundServiceItemRequest(SOUND_REMIND_CALLRING, REMIND_ATTR_NORMAL);//SOUND_REMIND_STANDBYM
			#endif
