...
static void WaitingPlayModeCreating(uint16_t msgId)
{
	...
	{
		APP_DBG("Waiting:Decoder created\n");
...
static void WaitingPlayModeStarting(uint16_t msgId)
{
	...
	{
		APP_DBG("Waiting:Decoder startted\n");
...
#endif
	{
		...
		if(!RemindSoundEmptySys())
		{
			...
			RemindSoundServicePlay(); //插播系统提示音
		}
		else
		{
			//无主音源时，未加提示音
		...
		if(!SoftFlagGet(SoftFlagDeepSleepRequest))
		{
			...
			RemindMixServiceItemRequest(SOUND_REMIND_KAIJI,0);//mainAppCt.SysRemind, 0); //插播系统提示音
			#endif
		}
#endif
	}
...
static void WaitingPlayEntrance(void * param)
{
...
#endif
#endif
	SoftFlagRegister(SoftFlagWaiting);// WaitingPlayPermissionMode处理后注销
...
#ifdef CFG_FUNC_REMIND_SOUND_EN
		RemindSoundServiceItemRequest(SOUND_REMIND_GUANJI, REMIND_ATTR_SYS);
#elif defined CFG_FUNC_REMIND_MIX_MODE
		RemindMixServiceItemRequest(SOUND_REMIND_GUANJI, TRUE);
#endif
	}
#endif
	while(1)
	{
		...
		if(!SoftFlagGet(SoftFlagWaitModeMask & (~SoftFlagWaiting))
#ifdef CFG_FUNC_REMIND_SOUND_EN
				&&  RemindSoundEmptySys()
#endif
				)
		{
#ifdef CFG_FUNC_REMIND_MIX_MODE
		if(RemindMixServiceRequestPlayStatus()==0)
#endif
	 {
#ifdef CFG_FUNC_REMIND_MIX_MODE
			SoftFlagDeregister(SoftFlagRemindMask);
#endif
			if(SoftFlagGet(SoftFlagWaiting))
			{
				SoftFlagDeregister(SoftFlagWaiting);
...
static void WaitingPlayRunning(uint16_t msgId)
{
	switch(msgId)
	{
		...
		case MSG_TASK_POWERDOWN:
			APP_DBG("Waiting:MSG receive PowerDown, Please breakpoint\n");
			SystemPowerDown();
			break;
#endif
...
bool WaitingPlayCreate(MessageHandle parentMsgHandle)
{
	...
	{
		APP_DBG("Waiting:app create fail!\n");
	}
	return ret;
}
