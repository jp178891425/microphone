...
extern void SysTickInitSet(void);
...
void Timer2Interrupt(void)
{
	...
	if((--DebugTwsTime) % DEBUG_COUNT_WIN == 0)
	{
		if(DebugTwsDataCount > DebugTwsDataMax)
		{
			DebugTwsDataMax = DebugTwsDataCount;
		}
		if(DebugTwsDataCount < DebugTwsDataMin)
		{
			DebugTwsDataMin = DebugTwsDataCount;
		}
		DebugTwsDataTotal += DebugTwsDataCount;
		DebugTwsDataCount = 0;
		if(DebugTwsTime == 0)
		{
			DebugTwsLog = TRUE;
			DebugTwsTime = DEBUG_SPEED_WIN;
		}
	}
#endif
}
...
void SystemClockInit(void)
{
	...
	Clock_Config(1, 24000000);
		...
		#if defined(CFG_CHIP_BP1048P2) || defined(CFG_CHIP_BP1048P4)
		Clock_PllLock(360000);//系统默认主频最高位360M
		#else
		Clock_PllLock(320000);//系统默认主频最高位320M
		#endif
		...
		extern void tws_work_mode_set(uint8_t mode);
		tws_work_mode_set(0);
	#else
		#if defined(CFG_CHIP_BP1048P2) || defined(CFG_CHIP_BP1048P4)
		Clock_PllLock(360000);//系统默认主频最高位360M
		Clock_APllLock(225792);
		Clock_USBClkDivSet(6);// bkd add for u disk 60M 2019.4.17
		#else
		Clock_PllLock(300000);//系统默认主频最高位300M
		Clock_APllLock(225792);
...
#else
	//注意：
	//系统默认主频最高位288M,最高可以超频到320M
	//超频到320之后
	//Core1V2必须提高到1.25V，设置是1.3V(不能保证所有芯片都能提高电压到1.3V)
	//此时芯片有一定概率存在异常风险（尤其是一些高温环境下工作）
	//芯片主频建议更具实际情况来调整，主频越小，芯片功能也会降低
		...
		Clock_USBClkDivSet(6);// bkd add for u disk 60M 2019.4.17
		#else
		Clock_PllLock(300000);//系统默认主频最高位360M
		Clock_APllLock(240000);
		Clock_USBClkDivSet(5);// bkd add for u disk 60M 2019.4.17
		#endif
		Clock_SysClkSelect(PLL_CLK_MODE);
		...
		#if defined(CFG_CHIP_BP1048P2) || defined(CFG_CHIP_BP1048P4)
		Clock_PllLock(360000);//系统默认主频最高位360M
		#elif defined(CFG_FUNC_MIC_KARAOKE_EN)
		Clock_PllLock(320000);//系统默认主频最高位320M
		#else
		Clock_PllLock(288000);//系统默认主频最高位288M
		#endif
		Clock_APllLock(240000);
...
#elif defined (MODIFY_USE_UART_DEBUG_DELAY)
	// do nothing
#else
...
void WakeupMain(void)
{
	...
	if (Clock_CoreClockFreqGet() > 320*1000000)
		SpiFlashInit(90000000, MODE_4BIT, 0, 1);
	else
		SpiFlashInit(96000000, MODE_4BIT, 0, 1);
...
#if defined(CFG_ADC_LEVEL_KEY_EN) || defined(CFG_RES_ADC_KEY_USE)
	SarADC_Init();
#endif
#if defined(CFG_RES_ADC_KEY_USE)
...
//#endif
    ...
    SystemClockInit();
    //主频超过288M时需要提升到1.25v，甚至更高的1.3V，降低芯片异常概率
    if(Clock_PllFreqGet() > 288000000)
    {
    	Power_LDO12Config(1250);
    }
#ifdef CFG_GPIO_RESET_HOLD_EN
...
#if FLASH_BOOT_EN
	Remap_DisableTcm();
#ifndef CHIP_AP10XX_P2
	Remap_InitTcm(FLASH_ADDR+0x10000, TCM_SIZE);
#else
	Remap_InitTcm(FLASH_ADDR+0x20000, TCM_SIZE);
#endif
#else
#ifndef CHIP_AP10XX_P2
...
{  
    ...
    APP_DBG("Driver Version: %s  Chip_Version: %x\n", GetLibVersionDriver(),Chip_Version());//
