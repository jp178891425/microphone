...
void BP_LoadInfo(void)
{
	...
	if(!ret)
	{
		...
		StoryVarInit();
		SaveStoryVar((void*)&bp_info.PlayerInfo.StoryInformation);
#endif
#endif
...
static bool SaveBakBPInfo(uint32_t BakNvmAddr, uint32_t NvmBakId, uint8_t SubBakId)	//SaveBakNVM
{
	...
	BP_BAK_WRAPPER *pBakWrapper = (BP_BAK_WRAPPER*)&Temp[0];
	BP_INFO * pBakInfo = (BP_INFO * )&Temp[sizeof(BP_BAK_WRAPPER)];

	memcpy((void*)pBakInfo, (void*)&bp_info, sizeof(BP_INFO));
#ifdef CFG_APP_CARD_PLAY_MODE_EN
	pBakInfo->PlayerInfo.PlayCardInfo.PlayTime = 0;
#endif
#ifdef CFG_APP_USB_PLAY_MODE_EN
	pBakInfo->PlayerInfo.PlayUDiskInfo.PlayTime = 0;
#endif
	CrcVal = GetCrc8CheckSum((uint8_t*)&pBakInfo, sizeof(BP_INFO)-1);
	pBakInfo->Crc = CrcVal;	//首先设置bp_info内部CRC

	//计算整个bp_info的crc
	CrcVal = GetCrc8CheckSum((uint8_t*)pBakInfo, sizeof(BP_INFO));
...
#ifdef	FUNC_SPECIFY_FOLDER_PLAY_EN	
				LoadStoryVar((void*)&BpPlayInfo->StoryInformation);
#endif
