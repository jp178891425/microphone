...
{
	if(AsyncIndex >= AUDIO_CORE_SOURCE_MAX_NUM)
		return ;
...
extern uint32_t GetValidFrameDataSize(void);
extern uint32_t GetValidSbcDataSize(void);
extern uint16_t BtHf_SendScoBufLenGet(void);
void AudioCoreSourceFreqAdjust(void)
{
	...
	if(FreqAsyncSource < AUDIO_CORE_SOURCE_MAX_NUM
	&& ((AudioCore.AudioSource[FreqAsyncSource].Enable)))// && AudioCore.AudioSource[FreqAsyncSource].FuncDataGetLen != NULL
	{
		...
		if(GetSystemMode() == AppModeBtAudioPlay)
		{
			if(GetBtManager()->a2dpStreamType == BT_A2DP_STREAM_TYPE_AAC)
				AsyncRemain += GetValidFrameDataSize();
			else
...
void AudioCoreSourceSRAInit(void)
{
	...
	resampler_farrow_init(&audioSourceAdjustCt->SraResFarCt, 2, POLYNOMIAL_ORDER);
...
uint16_t AudioSourceSRAProcess(int16_t *InBuf, uint16_t InLen)
{
	...
	if(resampler_farrow_apply(&audioSourceAdjustCt->SraResFarCt, (int16_t *)InBuf, (int16_t *)audioSourceAdjustCt->SraPcmOutnBuf, 128, 128 + AdjustVal) != RESAMPLER_FARROW_ERROR_OK)
	{
		return 0;
	}
  //debug only
...
{
	APP_DBG("open Sink SRA\n");
	if(audioSinkAdjustCt == NULL)
	{
		APP_DBG("audioSinkAdjustCt == NULL\n");
		return;
	}
	memset(audioSinkAdjustCt, 0, sizeof(AudioAdjustContext));

	//sra_init(&audioSinkAdjustCt->SraObj,2);//默认双声道
	resampler_farrow_init(&audioSinkAdjustCt->SraResFarCt, 2, POLYNOMIAL_ORDER);
...
uint16_t AudioSinkSRAProcess(int16_t *InBuf, uint16_t InLen)
{
	...
	if(resampler_farrow_apply(&audioSinkAdjustCt->SraResFarCt, (int16_t *)InBuf, (int16_t *)audioSinkAdjustCt->SraPcmOutnBuf, 128, 128 + AdjustVal) != RESAMPLER_FARROW_ERROR_OK)
	{
		return 0;
	}
