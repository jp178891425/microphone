...
const AUDIO_EFF_PARAMAS EFFECT_TAB[11]={
	...
	{EFFECT_MODE_HanMai_Slave,     HanMai_Slave,     sizeof(HanMai_Slave),    "HanMai_Slave"},
	{EFFECT_MODE_MoYin_Slave,      MoYin_Slave,      sizeof(MoYin_Slave),     "MoYin_Slave"},
	{EFFECT_MODE_NanBianNv_Slave,  NanBianNv_Slave,  sizeof(NanBianNv_Slave), "NanBianNv_Slave"},
	{EFFECT_MODE_NvBianNan_Slave,  NvBianNan_Slave,  sizeof(NvBianNan_Slave), "NvBianNan_Slave"},
	{EFFECT_MODE_WaWaYin_Slave,    WaWaYin_Slave,    sizeof(WaWaYin_Slave),   "WaWaYin_Slave"},
	#endif
#else
	...
	{EFFECT_MODE_USB_AEC,	       UsbAecBuf,  sizeof(UsbAecBuf), "UsbAecBuf"},
	#ifdef BT_TWS_SUPPORT
	{EFFECT_MODE_NORMAL_SLAVE,     Music_Slave,sizeof(Music_Slave),"Music_Slave"},
	#endif
#endif
	{0xffff,                       (uint8_t *)0x00, 0x00, " "}
};

#ifdef CFG_FUNC_DOWNDLOAD_EFF_TO_FLASH
//注意：名字后缀加入_F，以表示与code中的音效区别
const AUDIO_EFF_PARAMAS FLASH_EFFECT_TAB[5]={
	{0,        0,    0,    "HunXiang_F"},
	{1,        0,    0,    "DianYin_F"},
	{2,        0,    0,    "HanMai_F"},
	//-------don't modify--------------------------------//
	{3,	       0,    0x4000,    "AECBuf_F"},  // len=0x4000,don't move
	{4,	       0,    0x8000,    "UsbAecBuf_F"},//len=0x8000 don't move
	//---------------------------------------------//
};
#endif
...
EQFilterParams *eq_drc_filter_buf[2] =  {
		eq_drc_1_filter_buf,
		eq_drc_2_filter_buf,
};

EQ_DRCUnit *eq_drc_unit[2] =  {
		  &gCtrlVars.music_eq_drc_unit,
		  &gCtrlVars.rec_eq_drc_unit,
};
#endif

#if CFG_AUDIO_EFFECT_DRC_LEGACY_EN
const int16_t DRC_LEGACY_DEFAULT_TABLE[][3] =
{
	{CFG_AUDIO_EFFECT_MUSIC_DRC_EN,		},
	{CFG_MUSIC_DRC_MODE, 				},
	{CFG_MUSIC_DRC_CROSSOVER_FREQ, 		},
	{CFG_MUSIC_DRC_CROSSOVER_FQ1, 		},
	{CFG_MUSIC_DRC_CROSSOVER_FQ2, 		},
	{CFG_MUSIC_DRC_PREGAIN1, 			},
	{CFG_MUSIC_DRC_PREGAIN2, 			},
	{CFG_MUSIC_DRC_THRESHHOLD1, 		},
	{CFG_MUSIC_DRC_RATIO1, 				},
	{CFG_MUSIC_DRC_ATTACK_TIME1, 		},
	{CFG_MUSIC_DRC_RELEASE_TIME1, 		},
	{CFG_MUSIC_DRC_THRESHHOLD2, 		},
	{CFG_MUSIC_DRC_RATIO2, 				},
	{CFG_MUSIC_DRC_ATTACK_TIME2, 		},
	{CFG_MUSIC_DRC_RELEASE_TIME2,		},
	{CFG_MUSIC_DRC_THRESHHOLD3, 		},
	{CFG_MUSIC_DRC_RATIO3, 				},
	{CFG_MUSIC_DRC_ATTACK_TIME3, 		},
	{CFG_MUSIC_DRC_RELEASE_TIME3,		},
};
const  uint8_t  drc_legacy_unit_channel[2]={//1=mono,2=stereo
		2,//&gCtrlVars.music_drc_legacy_unit,
		2,//&gCtrlVars.mic_drc_legacy_unit,
};
DRCLegacyUnit *drc_legacy_unit_init[2]={
		&gCtrlVars.music_drc_legacy_unit,
		&gCtrlVars.mic_drc_legacy_unit,
};

#endif//end of  CFG_AUDIO_EFFECT_DRC_LEGACY_EN
////////////////////////////////////////////////
#if CFG_AUDIO_EFFECT_DYNAMIC_EQ_EN

const DynamicEqUnit  *DynamicEqArray[1]={
		 &gCtrlVars.music_dynamic_eq_unit,
};

void DynmmicEq_Free(DynamicEqUnit *unit)
{
    osPortFree(unit->ct);
    osPortFree(unit->eq_low.ct);
    osPortFree(unit->eq_high.ct);
    osPortFree(unit->eq_watch.ct);

    unit->ct            = NULL;
    unit->eq_low.ct     = NULL;
    unit->eq_high.ct    = NULL;
    unit->eq_watch.ct   = NULL;
}
...
void DynmmicEq_Disable(DynamicEqUnit *unit)
{
    ...
    unit->enable            = 0;
    unit->eq_low.enable     = 0;
    unit->eq_high.enable    = 0;
    unit->eq_watch.enable   = 0;
}
#endif
/*
****************************************************************
...
void CtrlVarsInit(void)
{
	...
	music_treb_bass_eq_unit      = NULL;
	#endif
	#ifdef CFG_FUNC_MUSIC_EQ_MODE_EN
	music_eq_mode_unit           = NULL;
	#endif
	#ifdef CFG_FUNC_MIC_TREB_BASS_EN
	mic_treb_bass_eq_unit  = NULL;
	#endif
	...
	gCtrlVars.music_delay_unit.bit_width     	 =  0;
    #endif
	...
	gCtrlVars.hfp_ns_unit.ct = 0;
	gCtrlVars.hfp_ns_unit.ns_level = 0;
	#endif

	gCtrlVars.eq_for_aec_unit.filter_params = eq_hfp_filter_buf;
	...
	gCtrlVars.auto_tune_unit.bit_width           = 0;
	...
	gCtrlVars.echo_unit.reserve 			     = CFG_MIC_ECHO_DIRECT_SND_EN;
	gCtrlVars.echo_unit.fc 						 = CFG_MIC_ECHO_CUTOFF_FREQ;
	gCtrlVars.echo_unit.high_quality 			 = 0;
	gCtrlVars.echo_unit.dry 					 = 50;
	gCtrlVars.echo_unit.wet 					 = 50;
	gCtrlVars.echo_unit.s_buf					 = NULL;
	gCtrlVars.echo_unit.bit_width			     = 0;
    #endif
	...
	gCtrlVars.pitch_shifter_unit.bit_width       = 0;
	...
	{
	int32_t min_hz;
	gCtrlVars.pitch_detector_unit.pitch_min = 55;//unit: HZ MIN = 48HZ
	gCtrlVars.pitch_detector_unit.pitch_max = 495;//unit: HZ,MAX= (gCtrlVars.sample_rate/2)HZ
	gCtrlVars.pitch_detector_unit.step_size = 2;// <= Windows_size
	gCtrlVars.pitch_detector_unit.confidence_threshold = 5000;//Range: 5000~10000 for 50.00% ~ 100%
	gCtrlVars.pitch_detector_unit.confidence = 0;//result
	gCtrlVars.pitch_detector_unit.window_size = 512;//option
	min_hz = ceil(gCtrlVars.sample_rate/gCtrlVars.pitch_detector_unit.pitch_min);
	min_hz <<= 1;//*2
	if(gCtrlVars.pitch_detector_unit.window_size < min_hz)
	{
	  APP_DBG("pitch_detector win size Default value = 512\n");
	  gCtrlVars.pitch_detector_unit.window_size = 512;//MAX=2048; MIN=2*ceil(sample_rate/pitch_min)
	}
	...
	#else
	gCtrlVars.pitch_detector_unit.pitch_min       = 50;
	gCtrlVars.pitch_detector_unit.pitch_max       = 340;
	gCtrlVars.pitch_detector_unit.window_size       = 1024*2;
	gCtrlVars.pitch_detector_unit.step_size       = 1024;
	gCtrlVars.pitch_detector_unit.confidence_threshold  = 9000; // 90.00%
	gCtrlVars.pitch_detector_unit.freq            = 0;
	gCtrlVars.pitch_detector_unit.freq_bak        = 0;
	#endif
	#endif
	...
	gCtrlVars.reverb_unit.bit_width              = 0;//only 16,24
	...
	gCtrlVars.plate_reverb_unit.bit_width        = 0;//only 16,24
	...
	gCtrlVars.reverb_pro_unit.width              =  0;////only 16,24
    #endif
	...
	gCtrlVars.freq_shifter_fine_unit.deltaf   		 = 666;
    #endif
	...
	gCtrlVars.howling_dector_fine_unit.q_min          = 50;
	gCtrlVars.howling_dector_fine_unit.q_max          = 500;
    #endif
    ...
    gCtrlVars.howling_dector_specified_unit.num_specified_filters   = 0;
    gCtrlVars.howling_dector_specified_unit.center_freq[0]            = 500;
    gCtrlVars.howling_dector_specified_unit.center_freq[1]            = 500;
    gCtrlVars.howling_dector_specified_unit.center_freq[2]            = 500;
    gCtrlVars.howling_dector_specified_unit.center_freq[3]            = 500;
    gCtrlVars.howling_dector_specified_unit.center_freq[4]            = 500;
    gCtrlVars.howling_dector_specified_unit.center_freq[5]            = 500;

    gCtrlVars.howling_dector_specified_unit.q[0]            = 45;
    gCtrlVars.howling_dector_specified_unit.q[1]            = 45;
    gCtrlVars.howling_dector_specified_unit.q[2]            = 45;
    gCtrlVars.howling_dector_specified_unit.q[3]            = 45;
    gCtrlVars.howling_dector_specified_unit.q[4]            = 45;
    gCtrlVars.howling_dector_specified_unit.q[5]           = 45;

    gCtrlVars.howling_dector_specified_unit.depth[0]            = 0;
    gCtrlVars.howling_dector_specified_unit.depth[1]            = 0;
    gCtrlVars.howling_dector_specified_unit.depth[2]            = 0;
    gCtrlVars.howling_dector_specified_unit.depth[3]            = 0;
    gCtrlVars.howling_dector_specified_unit.depth[4]            = 0;
    gCtrlVars.howling_dector_specified_unit.depth[5]            = 0;
    #endif
	...
	gCtrlVars.music_vb_unit.vb_type              =  0;
	#endif
	...
	gCtrlVars.chorus2_unit.bit_width               = 0;
	gCtrlVars.chorus2_unit.delay_length            = CFG_CHORUS_DELAY_LENGTH;
	gCtrlVars.chorus2_unit.mod1_depth              = CFG_CHORUS_MODULATION_DEPTH;
	gCtrlVars.chorus2_unit.mod1_rate               = CFG_CHORUS_MODULATION_RATE;
	gCtrlVars.chorus2_unit.mod2_depth              = CFG_CHORUS_MODULATION_DEPTH;
	gCtrlVars.chorus2_unit.mod2_rate               = CFG_CHORUS_MODULATION_RATE;
	gCtrlVars.chorus2_unit.dry                     = CFG_CHORUS_DRY;
	gCtrlVars.chorus2_unit.wet_1                   = CFG_CHORUS_WET;
	gCtrlVars.chorus2_unit.wet_2                   = CFG_CHORUS_WET;
    #endif
	...
	gCtrlVars.ping_pong_unit.bit_width          = 0;
    #endif
	...
	gCtrlVars.flanger_uint.delay_length   =  1;
	gCtrlVars.flanger_uint.mod_depth	  =  1;
	gCtrlVars.flanger_uint.mod_rate       =  100;
	gCtrlVars.flanger_uint.feedback 	  =  70;
	gCtrlVars.flanger_uint.dry	   		  =  59;//0-100
	gCtrlVars.flanger_uint.wet		      =  100;
	#endif

	#if CFG_AUDIO_EFFECT_OVERDRIVER_EN
	gCtrlVars.overdrive_unit.enable       = 0;
	gCtrlVars.overdrive_unit.threshold_compression = OVERDRIVE_THRESHOLD_COMPRESSION_MIN;
	#endif

	#if CFG_AUDIO_EFFECT_DISTORTION_EN
	gCtrlVars.distortion_unit.enable      = 0;
	gCtrlVars.distortion_unit.gain = 0;
	gCtrlVars.distortion_unit.dry = 0x08;
	gCtrlVars.distortion_unit.wet= 0x64;
	#endif
	...
	gCtrlVars.tsm_unit.tsm_en       =  0;
	gCtrlVars.tsm_unit.ct          = NULL;
	gCtrlVars.tsm_unit.speed_ratio = 16384;
    #endif
	...
	for(i = 0; i < sizeof(drc_unit_aggregate)/sizeof(drc_unit_aggregate[0]); i++)
	{
		...
		drc_unit_aggregate[i]->mode         = 0;//full
		drc_unit_aggregate[i]->cf_type      = 1;//b1
		drc_unit_aggregate[i]->q_l          = 724;//
		drc_unit_aggregate[i]->q_h          = 724;//
		drc_unit_aggregate[i]->fc[0]        = 300;//
		drc_unit_aggregate[i]->fc[1]        = 2000;//
		drc_unit_aggregate[i]->threshold[0] = 0;
		drc_unit_aggregate[i]->threshold[1] = 0;
		drc_unit_aggregate[i]->threshold[2] = 0;
		drc_unit_aggregate[i]->threshold[3] = 0;
		drc_unit_aggregate[i]->ratio[0]     = 100;
		drc_unit_aggregate[i]->ratio[1]     = 100;
		drc_unit_aggregate[i]->ratio[2]     = 100;
		drc_unit_aggregate[i]->ratio[3]     = 100;
		drc_unit_aggregate[i]->attack[0]    = 100;
		drc_unit_aggregate[i]->attack[1]    = 100;
		drc_unit_aggregate[i]->attack[2]    = 100;
		drc_unit_aggregate[i]->attack[3]    = 100;
		drc_unit_aggregate[i]->release[0]   = 1;
		drc_unit_aggregate[i]->release[1]   = 1;
		drc_unit_aggregate[i]->release[2]   = 1;
		drc_unit_aggregate[i]->release[3]   = 1;
		drc_unit_aggregate[i]->pregain[0]   = 4096;
		drc_unit_aggregate[i]->pregain[1]   = 4096;
		drc_unit_aggregate[i]->pregain[2]   = 4096;
		drc_unit_aggregate[i]->pregain[3]   = 4096;
	...
	gCtrlVars.biquad_unit.enable  = 0;
	gCtrlVars.biquad_unit.channel = 2;
	gCtrlVars.biquad_unit.reserve1  =0;
	gCtrlVars.biquad_unit.reserve2  =0;
	gCtrlVars.biquad_unit.use_float  =0;
	gCtrlVars.biquad_unit.filter_params        = biquad_filter_buf;
	gCtrlVars.biquad_unit.eq_params[0].enable  = 1;//
	gCtrlVars.biquad_unit.eq_params[0].type    = 0;
	gCtrlVars.biquad_unit.eq_params[0].f0      = 200;
	gCtrlVars.biquad_unit.eq_params[0].Q       = 724;
	gCtrlVars.biquad_unit.eq_params[0].gain    = 0;

	gCtrlVars.biquad_unit.filter_params->type    = 0;
	gCtrlVars.biquad_unit.filter_params->f0      = 200;
	gCtrlVars.biquad_unit.filter_params->Q       = 724;
	gCtrlVars.biquad_unit.filter_params->gain    = 0;
#endif


    #if CFG_AUDIO_EFFECT_BUTTERWORTH_EN
	gCtrlVars.music_butterworth_unit.ct             = 0;
	gCtrlVars.music_butterworth_unit.enable         = 0;
	gCtrlVars.music_butterworth_unit.filter_type    = 0;
	gCtrlVars.music_butterworth_unit.filter_order   = 1;
	gCtrlVars.music_butterworth_unit.fc             = 0;
	gCtrlVars.music_butterworth_unit.channel        = 0;
    #endif
    // -----------init---------------------------//
    #if CFG_AUDIO_EFFECT_DYNAMIC_EQ_EN

	for(i = 0; i < sizeof(DynamicEqArray)/sizeof(DynamicEqArray[0]); i++)
	{
		dynamic_eq_ptr =(DynamicEqUnit *) DynamicEqArray[i];

		dynamic_eq_ptr->ct                        = 0;
		dynamic_eq_ptr->enable                    = 0;
		dynamic_eq_ptr->low_energy_threshold      = -50;
		dynamic_eq_ptr->normal_energy_threshold   = -40;
		dynamic_eq_ptr->high_energy_threshold     = -10;
		dynamic_eq_ptr->attack_time               = 100;
		dynamic_eq_ptr->release_time              = 1000;
		dynamic_eq_ptr->channel                   = 0;
		//-------------low freq----------------------------------------------//
		dynamic_eq_ptr->eq_low.ct                =0;
		dynamic_eq_ptr->eq_low.enable		     = 0;
		dynamic_eq_ptr->eq_low.filter_count      = 10;
		dynamic_eq_ptr->eq_low.pregain		     = 0;
		dynamic_eq_ptr->eq_low.calculation_type = 0;//0=fix ,1=float
		dynamic_eq_ptr->eq_low.channel          = 0;
		dynamic_eq_ptr->eq_low.filter_params = &dynamic_eq_ptr->low_filter_buf[0];

		for(j = 0; j < dynamic_eq_ptr->eq_low.filter_count; j++)
		{
			dynamic_eq_ptr->eq_low.eq_params[j].enable			= 0;
			dynamic_eq_ptr->eq_low.eq_params[j].f0				= 800;
			dynamic_eq_ptr->eq_low.eq_params[j].Q				= 724;
			dynamic_eq_ptr->eq_low.eq_params[j].gain			= 0;
			dynamic_eq_ptr->eq_low.eq_params[j].type			= EQ_FILTER_TYPE_HIGH_SHELF;

			(dynamic_eq_ptr->eq_low.filter_params + j)->f0	    = 800;
			(dynamic_eq_ptr->eq_low.filter_params + j)->Q		= 724;
			(dynamic_eq_ptr->eq_low.filter_params + j)->gain	= 0;
			(dynamic_eq_ptr->eq_low.filter_params + j)->type	= EQ_FILTER_TYPE_HIGH_SHELF;
		}
		//-----------------hight freq------------------------------------------//
		dynamic_eq_ptr->eq_high.ct       =0;
		dynamic_eq_ptr->eq_high.enable		  = 0;
		dynamic_eq_ptr->eq_high.filter_count  = 10;
		dynamic_eq_ptr->eq_high.pregain		  = 0;
		dynamic_eq_ptr->eq_high.calculation_type = 0;//0=fix ,1=float
		dynamic_eq_ptr->eq_high.channel          = 0;
		dynamic_eq_ptr->eq_high.filter_params = &dynamic_eq_ptr->high_filter_buf[0];

		for(j = 0; j < dynamic_eq_ptr->eq_high.filter_count; j++)
		{
			dynamic_eq_ptr->eq_high.eq_params[j].enable			= 0;
			dynamic_eq_ptr->eq_high.eq_params[j].f0				= 800;
			dynamic_eq_ptr->eq_high.eq_params[j].Q				= 724;
			dynamic_eq_ptr->eq_high.eq_params[j].gain			= 0;
			dynamic_eq_ptr->eq_high.eq_params[j].type			= EQ_FILTER_TYPE_HIGH_SHELF;

			(dynamic_eq_ptr->eq_high.filter_params + j)->f0	    = 800;
			(dynamic_eq_ptr->eq_high.filter_params + j)->Q		= 724;
			(dynamic_eq_ptr->eq_high.filter_params + j)->gain	= 0;
			(dynamic_eq_ptr->eq_high.filter_params + j)->type	= EQ_FILTER_TYPE_HIGH_SHELF;
		}
		//-----------------watch eq------------------------------------------//
		dynamic_eq_ptr->eq_watch.ct                  =0;
		dynamic_eq_ptr->eq_watch.enable		         = 0;
		dynamic_eq_ptr->eq_watch.filter_count        = 10;
		dynamic_eq_ptr->eq_watch.pregain		     = 0;
		dynamic_eq_ptr->eq_watch.calculation_type    = 0;//0=fix ,1=float
		dynamic_eq_ptr->eq_watch.channel          = 0;
		dynamic_eq_ptr->eq_watch.filter_params       = &dynamic_eq_ptr->watch_filter_buf[0];

		for(j = 0; j < dynamic_eq_ptr->eq_watch.filter_count; j++)
		{
			dynamic_eq_ptr->eq_watch.eq_params[j].enable		= 0;
			dynamic_eq_ptr->eq_watch.eq_params[j].f0			= 800;
			dynamic_eq_ptr->eq_watch.eq_params[j].Q				= 724;
			dynamic_eq_ptr->eq_watch.eq_params[j].gain			= 0;
			dynamic_eq_ptr->eq_watch.eq_params[j].type			= EQ_FILTER_TYPE_HIGH_SHELF;

			(dynamic_eq_ptr->eq_watch.filter_params + j)->f0	= 800;
			(dynamic_eq_ptr->eq_watch.filter_params + j)->Q		= 724;
			(dynamic_eq_ptr->eq_watch.filter_params + j)->gain	= 0;
			(dynamic_eq_ptr->eq_watch.filter_params + j)->type	= EQ_FILTER_TYPE_HIGH_SHELF;
		}
	}
    #endif //end of CFG_AUDIO_EFFECT_DYNAMIC_EQ_EN
	...
	gCtrlVars.aec_gain_control_unit.mute		 = 0;
	gCtrlVars.aec_gain_control_unit.gain		 = 4096;
	gCtrlVars.aec_gain_control_unit.channel	 = 1;

	gCtrlVars.aec_mic_in_gain_unit.enable     = 0;
	gCtrlVars.aec_mic_in_gain_unit.mute		 = 0;
	gCtrlVars.aec_mic_in_gain_unit.gain		 = 4096;
	gCtrlVars.aec_mic_in_gain_unit.channel	 = 2;

	gCtrlVars.aec_phone_in_gain_unit.enable     = 0;
	gCtrlVars.aec_phone_in_gain_unit.mute		 = 0;
	gCtrlVars.aec_phone_in_gain_unit.gain		 = 4096;
	gCtrlVars.aec_phone_in_gain_unit.channel	 = 1;
    //#endif
#endif
	...
	for(i = 0; i < sizeof(eq_drc_unit)/sizeof(eq_drc_unit[0]); i++)
	{
	    ...
	    eq_drc_unit[i]->channel     = 0;

	    eq_drc_unit[i]->filter_params = eq_drc_filter_buf[i];

	    for(j =0;j < 10;j++)
		{
			eq_drc_unit[i]->eq_params[j].f0	     = 2000;//EQ_DEFAULT_TABLE[3 + j *4][i];
			eq_drc_unit[i]->eq_params[j].Q		 = 754;//EQ_DEFAULT_TABLE[4 + j *4][i];
			eq_drc_unit[i]->eq_params[j].gain	 = 4096;//EQ_DEFAULT_TABLE[5 + j *4][i];
			eq_drc_unit[i]->eq_params[j].type	 = 0;//EQ_DEFAULT_TABLE[6 + j *4][i];

			eq_drc_unit[i]->filter_params[j].f0	 = 2000;//EQ_DEFAULT_TABLE[3 + j *4][i];
			eq_drc_unit[i]->filter_params[j].Q	 = 754;//EQ_DEFAULT_TABLE[4 + j *4][i];
			eq_drc_unit[i]->filter_params[j].gain	 = 4096;//EQ_DEFAULT_TABLE[5 + j *4][i];
			eq_drc_unit[i]->filter_params[j].type	 = 0;//EQ_DEFAULT_TABLE[6 + j *4][i];
		}
		
		eq_drc_unit[i]->drc_mode = 0;
		eq_drc_unit[i]->cf_type  = 0;
		eq_drc_unit[i]->q_h      = 2000;
		eq_drc_unit[i]->q_l      = 100;
		eq_drc_unit[i]->fc[0]       = 2000;
		eq_drc_unit[i]->fc[1]       = 2000;
		eq_drc_unit[i]->release_tc[0] = 10;
		eq_drc_unit[i]->release_tc[1] = 10;
		eq_drc_unit[i]->release_tc[2] = 10;
		eq_drc_unit[i]->release_tc[3] = 10;
		eq_drc_unit[i]->ratio[0]      = 1;
		eq_drc_unit[i]->ratio[1]      = 1;
		eq_drc_unit[i]->ratio[2]      = 1;
		eq_drc_unit[i]->ratio[3]      = 1;
		eq_drc_unit[i]->threshold[0]  = 100;
		eq_drc_unit[i]->threshold[1]  = 100;
		eq_drc_unit[i]->threshold[2]  = 100;
		eq_drc_unit[i]->threshold[3]  = 100;
		eq_drc_unit[i]->attack_tc[0]  = 100;
		eq_drc_unit[i]->attack_tc[1]  = 100;
		eq_drc_unit[i]->attack_tc[2]  = 100;
		eq_drc_unit[i]->attack_tc[3]  = 100;
		eq_drc_unit[i]->pregain[0]    = 4096;
		eq_drc_unit[i]->pregain[1]    = 4096;
		eq_drc_unit[i]->pregain[2]    = 4096;
		eq_drc_unit[i]->pregain[3]    = 4096;
    }
#endif
	...
	for(i = 0; i < sizeof(drc_legacy_unit_init)/sizeof(drc_legacy_unit_init[0]); i++)
	{
		...
		drc_legacy_unit_init[i]->enable		    = DRC_LEGACY_DEFAULT_TABLE[0][0];
		drc_legacy_unit_init[i]->attack_tc[0]	= DRC_LEGACY_DEFAULT_TABLE[9][0];
		drc_legacy_unit_init[i]->attack_tc[1]	= DRC_LEGACY_DEFAULT_TABLE[13][0];
		drc_legacy_unit_init[i]->attack_tc[2]	= DRC_LEGACY_DEFAULT_TABLE[17][0];
		drc_legacy_unit_init[i]->fc		   	    = DRC_LEGACY_DEFAULT_TABLE[2][0];
		drc_legacy_unit_init[i]->mode			= DRC_LEGACY_DEFAULT_TABLE[1][0];
		drc_legacy_unit_init[i]->pregain1		= DRC_LEGACY_DEFAULT_TABLE[5][0];
		drc_legacy_unit_init[i]->pregain2		= DRC_LEGACY_DEFAULT_TABLE[6][0];
		drc_legacy_unit_init[i]->q[0]			= DRC_LEGACY_DEFAULT_TABLE[3][0];
		drc_legacy_unit_init[i]->q[1]			= DRC_LEGACY_DEFAULT_TABLE[4][0];
		drc_legacy_unit_init[i]->ratio[0]		= DRC_LEGACY_DEFAULT_TABLE[8][0];
		drc_legacy_unit_init[i]->ratio[1]		= DRC_LEGACY_DEFAULT_TABLE[12][0];
		drc_legacy_unit_init[i]->ratio[2]		= DRC_LEGACY_DEFAULT_TABLE[16][0];
		drc_legacy_unit_init[i]->release_tc[0]  = DRC_LEGACY_DEFAULT_TABLE[10][0];
		drc_legacy_unit_init[i]->release_tc[1]  = DRC_LEGACY_DEFAULT_TABLE[14][0];
		drc_legacy_unit_init[i]->release_tc[2]  = DRC_LEGACY_DEFAULT_TABLE[18][0];
		drc_legacy_unit_init[i]->threshold[0]	= DRC_LEGACY_DEFAULT_TABLE[7][0];
		drc_legacy_unit_init[i]->threshold[1]	= DRC_LEGACY_DEFAULT_TABLE[11][0];
		drc_legacy_unit_init[i]->threshold[2]	= DRC_LEGACY_DEFAULT_TABLE[15][0];

		drc_legacy_unit_init[i]->channel        = drc_legacy_unit_channel[i];
	}
#endif

    #if CFG_AUDIO_EFFECT_OVERDRIVER_POLY_EN
	gCtrlVars.overdrive_poly_unit.enable = 0;
    gCtrlVars.overdrive_poly_unit.gain = 0;
    gCtrlVars.overdrive_poly_unit.out_level = 50;
    #endif

    #if CFG_AUDIO_EFFECT_DISTORTION_DS1_EN
    gCtrlVars.distortion_ds1_unit.enable = 0;
    gCtrlVars.distortion_ds1_unit.distortion_level  =50;
    gCtrlVars.distortion_ds1_unit.out_level         = 50;
    #endif

    #if CFG_AUDIO_EFFECT_COMPANDER_EN
    gCtrlVars.compander_unit.enable       = 0;
    gCtrlVars.compander_unit.ct           = 0;
	gCtrlVars.compander_unit.threshold    = -4000;
	gCtrlVars.compander_unit.ratio_below  = 100;
	gCtrlVars.compander_unit.ratio_above  = 100;
	gCtrlVars.compander_unit.attack_time  = 5;
	gCtrlVars.compander_unit.release_time = 100;
	gCtrlVars.compander_unit.pregain      = 0;
	gCtrlVars.compander_unit.channel      = 2;
    #endif

    #if CFG_AUDIO_EFFECT_LOW_LEVEL_COMPRESSOR_EN
    gCtrlVars.mic_low_level_compressor_unit.enable       = 0;
    gCtrlVars.mic_low_level_compressor_unit.ct           = 0;
	gCtrlVars.mic_low_level_compressor_unit.threshold    = -6400;
	gCtrlVars.mic_low_level_compressor_unit.attack_time  = 10;
	gCtrlVars.mic_low_level_compressor_unit.release_time = 1000;
	gCtrlVars.mic_low_level_compressor_unit.gain         = 2140;
	gCtrlVars.mic_low_level_compressor_unit.channel      = 2;
    #endif

	#if CFG_AUDIO_EFFECT_HOWLING_GUARD_EN
	gCtrlVars.mic_howling_guard_unit.ct                               = NULL;
	gCtrlVars.mic_howling_guard_unit.enable                           = 0;
	gCtrlVars.mic_howling_guard_unit.saturation_threshold             = -50;
	gCtrlVars.mic_howling_guard_unit.high_freq_threshold              = 1000;
	gCtrlVars.mic_howling_guard_unit.high_freq_energy_ratio_threshold = 50;
	gCtrlVars.mic_howling_guard_unit.max_saturated_high_freq_duration = 1000;
	gCtrlVars.mic_howling_guard_unit.max_saturated_duration           = 1000;
	gCtrlVars.mic_howling_guard_unit.mute_period                      = 1000;
	gCtrlVars.mic_howling_guard_unit.noise_gate_threshold             = -50;
	#endif

	#if CFG_AUDIO_EFFECT_PHASE_SHIFTER_EN
	gCtrlVars.rec_phase_shifter_unit.ct = NULL;
	gCtrlVars.rec_phase_shifter_unit.enable = 0;
	gCtrlVars.rec_phase_shifter_unit.step_size = 128;
	gCtrlVars.rec_phase_shifter_unit.phase_shift = 0;
	gCtrlVars.rec_phase_shifter_unit.channel     = 1;
	#endif

    #if CFG_AUDIO_EFFECT_DRAPOST_EN
	gCtrlVars.dra_post_unit.enable        = 0;
	gCtrlVars.dra_post_unit.channel       = 0;
	gCtrlVars.dra_post_unit.ct            = NULL;

	gCtrlVars.dra_post_unit.nReserverd1   = 0;
	gCtrlVars.dra_post_unit.nReserverd2   = 0;

	gCtrlVars.dra_post_unit.nEffectActive = 0;
	gCtrlVars.dra_post_unit.nFreq1= 100;
	gCtrlVars.dra_post_unit.nFreq2= 200;
	gCtrlVars.dra_post_unit.nFreq3= 4000;
	gCtrlVars.dra_post_unit.nFreq4= 6000;
	gCtrlVars.dra_post_unit.nWidenCenter = 10;
	gCtrlVars.dra_post_unit.nWidenGain = 5;
	gCtrlVars.dra_post_unit.nTotalGain = 13;   			// 10x
	gCtrlVars.dra_post_unit.bCTCActive = 1;
	gCtrlVars.dra_post_unit.nCTCMode = 2;
	gCtrlVars.dra_post_unit.bUpmixActive = 1;
	gCtrlVars.dra_post_unit.fUpmixLRGainCoef = 12;     	// 10x
	gCtrlVars.dra_post_unit.fUpmixTlTrGainCoef = 13;   	// 10x
	gCtrlVars.dra_post_unit.fUpmixGainCoef = 14;       	// 10x
	gCtrlVars.dra_post_unit.fSpeechEnhancementGain = 16;  // 10x
	gCtrlVars.dra_post_unit.bVirtualBassActive = 1;
	gCtrlVars.dra_post_unit.nVBCutOffFreq = 100;
	gCtrlVars.dra_post_unit.fVBGain = 6;
	gCtrlVars.dra_post_unit.nLowFreq1 = 0;//30;
	gCtrlVars.dra_post_unit.nLowFreq2 = 60;
	gCtrlVars.dra_post_unit.nHarmCutOff = 0;//240;
	gCtrlVars.dra_post_unit.fScale = 10;					// 10x
	gCtrlVars.dra_post_unit.fGain_f1 = 10;					// 10x
	gCtrlVars.dra_post_unit.nVolume = 3;
	gCtrlVars.dra_post_unit.nDistance = 4;
	gCtrlVars.dra_post_unit.nRoomMode = 1;
    #endif

    #if CFG_AUDIO_EFFECT_DC_BLOCKER_EN
	gCtrlVars.dc_blocker_unit.ct = 0;
	gCtrlVars.dc_blocker_unit.enable = 0;
	gCtrlVars.dc_blocker_unit.channel  =0;
    #endif
	#if CFG_AUDIO_EFFECT_VIRTUAL_SURROUND_EN
	gCtrlVars.virtual_surround_unit.ct = 0;
	gCtrlVars.virtual_surround_unit.enable = 0;
	gCtrlVars.virtual_surround_unit.channel  =0;
	gCtrlVars.virtual_surround_unit.SampleRate =CFG_PARA_SAMPLE_RATE;
	#endif

    #if CFG_AUDIO_EFFECT_NOISE_GENERATOR_EN
    gCtrlVars.noise_generator_unit.ct = 0;
    gCtrlVars.noise_generator_unit.enable = 0;
    gCtrlVars.noise_generator_unit.channel  =0;
    gCtrlVars.noise_generator_unit.noise_type = 0;
    gCtrlVars.noise_generator_unit.amplitude = 0;
    #endif

	#if CFG_AUDIO_EFFECT_LRBALANCER_EN
    gCtrlVars.music_lr_balancer.ct = NULL;
    gCtrlVars.music_lr_balancer.enable = 0;
    gCtrlVars.music_lr_balancer.channel =0;
    gCtrlVars.music_lr_balancer.balance =0;
	#endif
	...
	else
	{
	    ...
	    music_treb_bass_eq_unit      = &gCtrlVars.music_eq_drc_unit;
        #else
	    music_treb_bass_eq_unit      = &gCtrlVars.music_pre_eq_unit;
        #endif
	#endif
	#ifdef CFG_FUNC_MUSIC_EQ_MODE_EN
	music_eq_mode_unit           = &gCtrlVars.music_out_eq_unit;
	#endif
	#ifdef CFG_FUNC_MIC_TREB_BASS_EN
	mic_treb_bass_eq_unit        = &gCtrlVars.mic_out_eq_unit;
	#endif

#endif  //end of AUDIO_EFFECT_EN
}
...
void GlobalSampeRateSet(void)
{
	...
	{
	    if(gCtrlVars.sys_sample_rate > sizeof(SupportSampleRateList)/sizeof(SupportSampleRateList[0]) )
    	{
			gCtrlVars.sys_sample_rate = 7;
    	}
...
void DefaultParamgsInit(void)
{
    ...
    #ifdef	CFG_FUNC_AUDIO_EFFECT_EN	
	gCtrlVars.crypto_en           = CFG_COMMUNICATION_CRYPTO&0x01;
...
****************************************************************
*/
extern const unsigned char BEEP_44K_TAB1[2784];
extern const unsigned char  BEEP_44K_TAB[41550-80];//vol max
