...
/*
 * DelayTime: set 0      解除SINK MUTE
 *                1      使能SINK MUTE，并且不需要延时解除MUTE
 *                other  使能SINK MUTE，并且在延时设置的时间后解除MUTE；单位（Ms）
 */
...
void AudioMusicVolUp(void)
{
...
#endif
	{
		...
		if(mainAppCt.MusicVolume == CFG_PARA_MAX_VOLUME_NUM)
		{
			...
			if (!SoftFlagGet(SoftFlagDecoderRemind))
			{
				...
				RemindSoundServiceItemRequest(SOUND_REMIND_MAX_VOL, REMIND_ATTR_NORMAL);
			}
			#else			
			#ifdef CFG_FUNC_REMIND_MIX_MODE
			gCtrlVars.remind_type = REMIND_TYPE_KEY;
			RemindMixServiceItemRequest(SOUND_REMIND_MAX_VOL,TRUE);
			#endif
			#endif
		}
	...
	AudioCoreSourceVolSet(USB_MIX_SOURCE_NUM, gSysVolArr[mainAppCt.gSysVol.AudioSourceVol[APP_SOURCE_NUM]], gSysVolArr[mainAppCt.gSysVol.AudioSourceVol[APP_SOURCE_NUM]]);
#endif
#ifdef CFG_FUNC_REMIND_MIX_MODE
	AudioCoreSourceVolSet(MIX_REMIND_SOURCE_NUM, gSysVolArr[mainAppCt.gSysVol.AudioSourceVol[MIX_REMIND_SOURCE_NUM]], gSysVolArr[mainAppCt.gSysVol.AudioSourceVol[MIX_REMIND_SOURCE_NUM]]);
#endif
...
void SystemAppSourceVolSet(void)
{
	AudioCoreSourceVolSet(APP_SOURCE_NUM, gSysVolArr[mainAppCt.gSysVol.AudioSourceVol[APP_SOURCE_NUM]], gSysVolArr[mainAppCt.gSysVol.AudioSourceVol[APP_SOURCE_NUM]]);
}
	...
	{				
		...
		switch(AdcLevelCh)
		{
			...
			case 2://ADC LEVEL Channel 2
				#ifdef CFG_FUNC_MIC_KARAOKE_EN	
			    mainAppCt.ReverbStepBak = AdcValue*2;
				//ReverbStepSet(mainAppCt.ReverbStep);
