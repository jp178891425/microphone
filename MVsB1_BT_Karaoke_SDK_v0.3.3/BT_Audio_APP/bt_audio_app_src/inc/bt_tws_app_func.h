...
#define BT_TWS_EVENT_DISCON_TIMEOUT		0x40	//发起断开行为,清除配对记录,超时断开标志； 500ms

#define BT_TWS_EVENT_PEER_PAIRING		0x80	//TWS组网(协助)
...
typedef struct _BtTwsAppCt
{
	...
	uint32_t			btTwsDisconTimeout;  //关联BT_TWS_EVENT_DISCON_TIMEOUT    //发起断开行为,清除配对记录,超时断开标志； 500ms

	uint32_t			btTwsPeerPairingTimeoutCnt;
}BtTwsAppCt;
