...
#define FILE_NAME_MAX							65534//(1~65534) //录音文件名编号范围。根据录音先后顺序命令。
...
#pragma pack(1)
typedef struct _REC_BLOCK_HEADER
{
	uint32_t			Index;
	uint32_t			FileSize;
}REC_BLOCK_HEADER;
#pragma pack()
#define		REC_BLOCK_ADDR				(EXTERN_BLOCK_SIZE - sizeof(REC_BLOCK_HEADER))
#define		REC_BLOCK_INDEX_ADDR		(EXTERN_BLOCK_SIZE - sizeof(REC_BLOCK_HEADER))// +
#define		REC_BLOCK_INDEX_LEN			sizeof(((REC_BLOCK_HEADER *)0)->Index)
#define		REC_BLOCK_SIZE_ADDR			(EXTERN_BLOCK_SIZE - 4)//sizeof(&((REC_BLOCK_HEADER *)0)->FileSize)
#define		REC_BLOCK_SIZE_LEN			sizeof(((REC_BLOCK_HEADER *)0)->FileSize)

#define		FLASH_NONE					0xFFFFFFFF//flash未写入
#define		FLASH_FILE_MAX				(EXTERN_BLOCK_SIZE - sizeof(REC_BLOCK_HEADER))
#endif
