...
typedef struct _MEDIA_PLAYER_CONTROL_
{
	...
	MemHandle	FlashMemHandle;
	uint16_t	BlockList[CFG_PARA_REC_MAX_FILE_NUM]; //录音文件队列 Block为下标，
	uint8_t		PlayList[CFG_PARA_REC_MAX_FILE_NUM]; //录音文件队列 映射block index
	uint32_t	FileOffset;
	uint32_t	FileSize;
#endif
...
//	uint8_t		IsBPMached;			// 是否FS已匹配了断点信息，主要用于防止保存无效的断点信息(播放模式初始化失败后，需要改标志判断是否保存断点信息)
	...
	uint8_t     PreNextPlayFlag;    //上下曲播放标志
#endif
